...
> 约10分钟。按问题递进组织内容，不预设PPT页码。页面保留证据，讲稿简要说明每一步怎样引出下一步。
...
首先做最直接的检查：把 26 个候选值都扫描一遍。当前参数域里的最低点是 0.06，但 0.1 的误差只比它高约 0.93%，所以更换统一常数带来的收益有限。右侧分解说明，偏移量在偏差与方差之间形成折中，并非越大越好。接下来就问：0.1 在这里接近最佳，是不是与这个参数域有关？

## 0.1 接近最佳，是否只是当前参数域的结果？

**页面上写**

保持形状参数窗口宽度不变，将窗口由 $[1.50,2.50]$ 平移至 $[4.00,5.00]$，重新扫描各窗口中的统一偏移量。

![固定宽度参数域与统一取值](assets/study01/fig3_beta_domain_restored_3d.png)

**结果：最佳统一偏移量由 0.18 移至 0.04。**

统一值是给定参数域和设计权重下的平均风险折中，0.1 不是脱离参数域的普适最优值。

**讲稿**

这里固定窗口宽度，只改变形状参数域的位置，最佳统一值就从 0.18 移到了 0.04。这说明，0.1 在当前混合域中合适，并不意味着换一个参数域后仍然最佳。既然参数条件会影响选择，那么按参数条件分别选，能不能进一步改善？
