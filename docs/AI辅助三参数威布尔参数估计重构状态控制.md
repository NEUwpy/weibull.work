...
本轮阶段：S4.9 MDM 默认实现与可视化一致性改造

做了什么：

- 阅读最新 MDM 有解性研究稿和 182-046 原文关键段落，确认默认 MDM 应保持离散搜索过程；
- 在 `python/methods/mdm.py` 中将旧两段均匀 gamma 网格改为 `geometric_from_tmin`，保留 `gamma_steps` 作为过程采样密度；
- 在默认 MDM 中加入 `gamma >= 0` 约束边界规则：无约束交点落在负半轴时返回 `gamma_hat = 0`，trace 写入 `solution_strategy = "truncated_at_zero"`；
- 更新 MDM 主可视化、offset 敏感性分析、梯度图参考线、算法说明和程序流程 JSON；
- 移除生产 runner 对历史 MDM 变体的依赖，并将变体实现、变体测试和 S4.5/S4.7 相关研究脚本归档到 `_archive/S4.9_MDM研究变体已完成/`；
- 新增 `python/tests/test_mdm_s49.py`，覆盖旧 failure 样本的边界截断和正常 offset-root trace 括区。

验证结果：

- `python -m pytest tests -q`：49 个测试通过；
- `npx tsc --noEmit`：通过；
- `python -m json.tool src/data/method_flows/mdm.json`：通过。

下一轮建议：

- 用 S4.9 后的默认 `mdm` 作为 baseline，按 S2R 指标重算 S4.7；
- 在正式重算前，可人工打开 MDM 方法页检查 `offset_root` 与 `truncated_at_zero` 两类样本的过程图标记。
