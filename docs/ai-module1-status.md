...
> **更新时间**: 2026-04-27
> **状态**: 前端页面重构完成（路线分离），所有 Tab 已填充数据，M1-R1/M1-R2 模型已训练，M1-R2 评估完成
...
- [x] 路线 2 重新评估：✅ evaluate_route2.py 已用 β={1,2,5} 跑完，数据已同步到前端
...
| 4 | 迭代过程 | ✅ 已完成 | δ 收敛轨迹折线图、逐步详情表（route2_iteration_traces.csv） |
...
✅ 已完成。evaluate_route2.py 已用 β={1,2,5} 跑完，生成 route2_convergence.csv (2250 条)、route2_iteration_traces.csv (1068 条)、route2_comparison.csv。

### 8.3 其他局限

1. **无截尾处理** — 只支持完全样本（Type II censoring 等未处理）
2. ~~前端页面待重构~~ — ✅ 已完成，拆分为 m1-r1/m1-r2 两个子页面
3. ~~param-accuracy 数据缺失~~ — ✅ 已完成，generate_param_accuracy.py 已生成三-way 对比数据
4. ~~M1-R2 迭代数据缺失~~ — ✅ 已完成，evaluate_route2.py 已输出迭代轨迹
...
| 5 | ~~param-accuracy 数据~~ | ✅ 已完成：generate_param_accuracy.py 生成三-way 对比 | ✅ |
