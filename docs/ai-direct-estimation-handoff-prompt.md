...
4. `C:\weibull\docs\ai-direct-estimation-v2-plan.md` — **V2 规划（本次任务）**
...
**当前参数空间**（V1）：
- β∈{0.5, 1.0, 2.0, 3.0, 5.0}, η∈{100, 500, 1000, 3000, 5000}, **γ=0（固定）**, n∈{5,7,10,15}, MC=500
...
详见 `C:\weibull\docs\ai-direct-estimation-v2-plan.md`，核心内容：

**任务 1：参数空间扩展**
- γ 从 {0} 扩展为 {0, 50, 100, 200}
- 总组合从 100→400，总样本从 50k→200k
- 需要重新生成数据 + 重新训练全部 8 个方案

**任务 2：泛化验证**
- 新建 `generate_test_data.py` 生成独立测试集
- 测试集参数组合分三类：组内(ig)、插值(ip)、外推(ex)
- 插值：β∈{0.75,1.5,2.5,4}, η∈{300,750,2000,4000}, γ∈{25,75,150}
- 外推：β∈{0.3,8,10}, η∈{50,8000,10000}, γ∈{-50,300,500}
- 新建 `evaluate_generalization.py` 对测试集做推理 + 按类型分组计算精度
- 输出 `generalization_metrics.json`

**任务 3：前端更新**
- VerificationTab 精度汇总表增加 validation_type 切换：[组内] [插值] [外推]
- PerformanceTab 精度分解表增加 validation_type 切换
- 散点图可选按 validation_type 着色
...
├── generate_test_data.py         # [待建] 泛化测试数据生成
├── train_model.py                # PyTorch 训练脚本（支持全部 8 种预处理）
├── evaluate_generalization.py    # [待建] 泛化评估脚本
└── data/                         # 训练数据 CSV
...
public/ai/data/                   # 前端加载的 metrics JSON + CSV
```
