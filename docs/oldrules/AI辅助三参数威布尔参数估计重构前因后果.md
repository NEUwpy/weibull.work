...
> 当前状态：本文是原始重构背景说明。当前执行路线已先完成默认 MDM、统一标准指标、公共蒙特卡洛流水线和 full-v1 baseline；继续开发以 `docs/AI辅助三参数威布尔参数估计重构当前路线图.md` 为入口。
...
```text
AI辅助三参数威布尔参数估计重构当前路线图.md
  作为继续开发和新窗口接手入口，说明已完成内容、路线偏移原因和下一步里程碑。

AI辅助三参数威布尔参数估计重构与实验设计总纲.md
  保留原始研究意图和模块划分，不作为当前逐条执行计划。
```

两者都不是最终代码设计文档。代码实现仍必须以当前代码结构、`02-规则.md`、`python/studies/common/README.md` 和实际验证结果为准。
