...
4. **分位数估计误差**：低尾概率 (F=0.01%, 0.1%, 1%, 10%) 下分位数估计的准确性如何？

### 分位数误差分析

分位数是可靠性工程中的关键指标，用于确定产品在设计寿命内的失效概率。

**计算公式**: `t(F) = γ + η × (-ln(1-F))^(1/β)`
...
3. **分位数误差表**: F=0.01%, 0.1%, 1%, 10% 的相对误差和 RMSE
4. **参数估计概率密度分布**: β、η、γ 的 KDE 分布对比
