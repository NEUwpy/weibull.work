...
id: demo-1
name: '示例1: WMLE多维度研究'
description: 研究 WMLE 方法在不同参数设置下的估计性能（单变量+双变量模式）
method: wmle
defaults:
  beta: 2.0
  eta: 1000
...
使用 WMLE（加权极大似然估计）估计三参数威布尔分布参数，评估不同因素对估计精度的影响。

与 MDM 不同，本研究采用"单变量+双变量"模式，避免全交叉组合带来的数据爆炸。

## 研究目标

1. 评估 WMLE 算法在不同参数组合下的估计性能
2. 分析 WMLE 相比传统 MLE 的偏差修正效果
