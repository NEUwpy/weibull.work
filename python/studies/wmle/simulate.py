...
配置文件格式 (config.md)：
    配置文件位于 public/studies/wmle/<study_id>/config.md
...
def run_wmle_estimation(sample: np.ndarray):
    """
    运行 WMLE 估计
    返回: (est_beta, est_eta, est_gamma, r2, status) 或 None (失败时)
    """
    try:
        algo = WMLE(sample.tolist())
...
def run_single_chunk(
    ...
    with open(output_path, 'w', newline='', encoding='utf-8') as f:
        ...
        for sim_id in range(1, mc_runs + 1):
            ...
            # WMLE 估计
            result = run_wmle_estimation(sample)
...
def main():
    parser = argparse.ArgumentParser(description='WMLE 方法通用仿真脚本 (单变量+双变量模式)')
    ...
    # 定位配置文件
    config_path = PROJECT_ROOT / 'public' / 'studies' / 'wmle' / args.study_id / 'config.md'
