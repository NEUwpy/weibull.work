...
import { loadCSV, schemeValidationPath, groupBy, DirectEstimationValidationRow } from '@/lib/ai-data'
  ...
  predictions.forEach((rows, n) => {
    ...
    groups.forEach((groupRows) => {
      ...
      for (const row of groupRows.slice(0, 3)) {
        cases.push({
          n,
          true_beta: row.true_beta,
          true_eta: row.true_eta,
          true_gamma: row.true_gamma,
          pred_beta: row.pred_beta,
          pred_eta: row.pred_eta,
          pred_gamma: row.pred_gamma,
        })
      }
    })
  })
