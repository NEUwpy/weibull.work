...
import { BookOpen, ChevronDown, GitCommit, Table2, AlertTriangle, CheckCircle, Info, Eye, EyeOff } from 'lucide-react'
import { cn } from '@/lib/utils'
import { useCaseList } from '@/hooks/useCaseList'

interface Case8ViewerProps {
  caseId: string
...
export default function Case8Viewer({ caseId, onCaseChange }: Case8ViewerProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setIsLoading(true)
        const res = await fetch('/case-studies/mdm/case8/data.json')
    ...
    return (
      <div className="bg-white rounded-2xl border border-slate-200 p-12">
        <div className="flex flex-col items-center justify-center">
          ...
          <p className="text-slate-600 font-bold">加载案例8数据中...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-2xl p-6 border border-indigo-200">
        <h2 className="text-xl font-bold text-slate-800 mb-2">案例8: β搜索方式对比研究</h2>
        <p className="text-sm text-slate-600 mb-2">
          数据来源: 实际样本 (n={data.data.length}) | β搜索方式: 固定步长 {data.beta_step || 0.05}
        </p>
        <div className="flex items-center gap-2 text-xs text-indigo-600 bg-indigo-100 px-3 py-1.5 rounded-lg w-fit">
          <Info size={14} />
          <span>与案例7的区别: β用固定步长遍历而非Brent优化</span>
        </div>
      </div>
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-xs text-slate-500 mt-2">
          β使用固定步长{traceData.beta_step || 0.05}遍历搜索。蓝色虚线: γ={traceData.optimal_gamma?.toFixed(2)}
        </p>
      </div>
      ...
      <div>
        ...
        <p className="text-xs text-slate-500 mt-2">
          ...
          颜色从红色(低γ)渐变到紫色(高γ)。每条曲线的最低点对应该 γ 的最优 β。
        </p>
      </div>
      ...
      <div>
        ...
        <p className="text-xs text-slate-500 mt-2">
          每个 γ 对应的最小标准差 σ_min（通过固定步长 {traceData.beta_step || 0.05} 遍历 β 得到）。
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-xs text-amber-600 bg-amber-50 px-2 py-1 rounded-md font-mono">
            最优 β = {traceData.optimal_beta?.toFixed(3) ?? '?'}
          </span>
        </div>
        <div className="h-[280px] w-full">
          {sliceData ? (
        ...
        <p className="text-xs text-slate-500 mt-2">
          在当前 γ={activeGamma.toFixed(0)} 下，σ_η 随 β 的变化曲线。
          <span className="text-amber-600 ml-1">注意：β 只能取步长 {traceData.beta_step || 0.05} 的整数倍</span>，
          所以最优 β 可能与真实最优值有偏差。
        </p>
      </div>
