...
export default function Demo2Viewer() {
  ...
  useEffect(() => {
    const loadConfig = async () => {
      try {
        setIsLoading(true)
        const res = await fetch('/studies/mle/demo2/config.md')
  ...
  useEffect(() => {
    const loadConvergenceData = async () => {
      try {
        const res = await fetch('/studies/mle/demo2/convergence.csv')
        if (!res.ok) return
        const text = await res.text()
        const lines = text.trim().split('\n')
        if (lines.length < 2) return
    ...
    const loadData = async () => {
      try {
        ...
        const dataPromises = selectedSampleSizes.map(async (n) => {
          const res = await fetch(`/studies/mle/demo2/chunks/n${n}.csv`)
          if (!res.ok) return []
          const text = await res.text()
  ...
  // 用于密度图的数据（需要包含 sample_size 字段）
  const densityData = csvData.filter(row => row.sim_id <= selectedMcRuns)
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-sm text-slate-500">
            ...
            <span className="font-bold text-slate-600">3</span> 个固定
          </div>
        </div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-blue-50 rounded-xl p-4 border border-blue-200">
            <h4 className="text-sm font-bold text-blue-800 mb-3">MLE 算法参数</h4>
            <div className="space-y-2 text-sm">
              ...
              {config.calculation?.gammaBounds && (
                <div className="flex justify-between">
                  <span className="text-slate-600">γ 搜索范围</span>
                  <span className="font-mono font-bold text-blue-700">[0, {config.calculation.gammaBounds[1]}×t<sub>min</sub>]</span>
                </div>
              )}
              {!config.calculation?.betaBounds && !config.calculation?.gammaBounds && (
                <div className="text-slate-500 text-xs">使用默认参数</div>
              )}
            </div>
          </div>
        </div>
              ...
              return (
                <ChartCard key={param} title={`图 ${idx + 1}: ${param}估计值分布`}>
                  <BoxPlotChart
                    ...
                    xLabel="n"
                    trueValue={chartConfig.trueValue}
                  />
                </ChartCard>
              )
            })}
