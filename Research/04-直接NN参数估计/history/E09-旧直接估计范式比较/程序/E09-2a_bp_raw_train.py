...
from bp_model import create_bp_raw_model
...
def train_single_n(n: int) -> dict:
    ...
    X_train, y_train = prepare_raw_input(df_train, n)
    X_val, y_val = prepare_raw_input(df_val, n)
    X_test, y_test = prepare_raw_input(df_test, n)
    ...
    # 5. 保存模型
    model_path = os.path.join(MODEL_DIR, f"E09-2a_bp_raw_n{n}.pt")
    trainer.save(model_path)
    print(f"模型已保存: {model_path}")

    # 6. 保存训练历史
    history_df = pd.DataFrame(history)
    history_df.to_csv(os.path.join(OUTPUT_DIR, f"E09-2a_history_n{n}.csv"), index=False)
    ...
    test_results.to_csv(os.path.join(OUTPUT_DIR, f"E09-2a_test_n{n}.csv"), index=False)
...
def main():
    ...
    summary_path = os.path.join(OUTPUT_DIR, "E09-2a_summary.csv")
