...
| `04-直接NN参数估计/` | 直接神经估计何时优于结构化传统估计、优势来自哪里、训练域改变后边界如何变化 | `ACTIVE` | `INDEPENDENT` | 已完成域内/域外、样本量、尺度、传统方法同协议比较及训练域宽度与位置分离实验；当前收口点估计的条件优势、机制与失效边界，回退规则不作为完成条件，区间评价后置，详见本研究 README |
