...
- r1/r2/r3 均永久 `blocked/aborted`，禁止恢复、删除、覆盖、续接、复制状态或拼接证据；r3 的 3 个成功 fit 只作为失败现场保留。
...
4. 确认当前 checkout 是从最终 clean、pushed `main` 创建的稳定 formal 分支；运行期间不得 pull、merge、切换 HEAD 或由其他 agent 修改该目录。
5. 重跑配置与冻结哈希校验，记录 protocol、search、amendment、matrix 和 effective config 的实际 SHA；必须与授权及冻结权威完全一致。
6. 重新确认运行时间风险。2026-07-26 投影为 1,373,822.99 秒（约 381.6 小时），启动时必须基于当前机器与执行策略复核。
7. 再次确认 test state 为 `sealed`、`test_access_count == 0`，且没有 approval/unseal/test 动作或产物。
...
- 任何步骤要求复用 r1/r2/r3，或提前进入 A-E3/A-E2；
