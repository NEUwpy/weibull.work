...
**Date**: 2026-07-20 (two REVISE passes after Codex review of `546df8a`)
...
> force-rerun whose executing script content is authoritatively identified by `code_sha256 =
> 7e28943bc520…` (verified to match the committed script at tip `14a166d`), with a standalone
> `SHA256SUMS` over source data + code + all results (150 entries, LF-normalised so it verifies in
> any checkout); (2) `checkpoint_valid` hardened (full identity + fingerprint + content checks);
> (3) `near_optimal_diagnostics.csv` now carries `seed`/`fold`/`model_id`; (4) this report's factual
> error and out-of-bounds interpretation corrected (see Erratum below). Numbers are unchanged
> (deterministic re-run). The final artifact commit on the branch is `b5e217f`; this report is
> `14a166d`.
...
| Remote branch | `origin/study01RAW @ 14a166d` (this report's commit; all preceding commits reachable) |
| Artifact commit | `b5e217f` — final committed form of the regenerated artifacts (code + results, a squashed rebase of intermediate REVISE steps `2d5e5ac` → `5719a9a` → `6c06208b`; those SHAs were local-only and do not resolve on the remote) |
| Authoritative code binding | `code_sha256 = 7e28943bc520…` (recorded in `manifest.json`, verified to match the LF-normalised content of `run_E3b_RAW_specialist.py` at `b5e217f` and `14a166d` — this is the script that actually executed the clean rerun) |
| Authoritative artifact binding | `SHA256SUMS` (150 entries, LF-normalised content fingerprints; verifies with the contract test on any checkout; plain `sha256sum -c` works on LF-native checkouts; on Windows CRLF checkouts the contract test normalises before hashing) |
...
**Command** (clean rerun, deterministic):
```bash
...
**Git-commit provenance note.** `manifest.json` records `"git_commit": "6c06208b…"` because
`get_git_metadata()` captured `HEAD` during the final clean rerun, which at that instant was an
intermediate squash (`6c06208b`, local-only; not pushed). That SHA does not resolve on the remote.
The authoritative code identity is `code_sha256 = 7e28943bc520…` — the SHA-256 of the LF-normalised
`run_E3b_RAW_specialist.py` content — which is verified to match the committed file at both
`b5e217f` (artifact commit) and `14a166d` (this report). Together with `SHA256SUMS` covering every
source, code, and result artifact, the provenance is cryptographically closed without depending on
unreachable intermediate SHAs.
...
- **[REVISE]** `SHA256SUMS` present and every entry verifies against disk (150 entries: 46 source, 3 code, 101 result; LF-normalised; plain `sha256sum -c` works on LF-native checkouts; on Windows CRLF checkouts the contract test normalises before hashing)
