...
### 2026-07-20 R6（Codex REVISE 之后，alias-chain + accreditation preflight 两缺口闭合棒）

Codex 复审 = REVISE：已批准且无需重做——option (a) 迁出、`4d5c9cd` 的路径迁出/冻结 matrix 派生/n 恢复/failed-fit 处理、`28b1d18` 的 349-smoke PASSED 证据。本棒只闭合两个 fail-closed 缺口，不重跑 2h smoke（未改 staged/selection 逻辑）。起点 `origin/claude/study02-a-20260715 @ 28b1d18`（即上一棒的最终 tip——上一棒共 **3** 个提交 `4d5c9cd`/`e9f455a`/`28b1d18`，纠正之前"4 个提交"误记）。

**已完成并提交（commit `cd4d04a`，`fix(study02a):`）：**
- **缺口 1（目录 alias-chain）**：`_validate_selection_point_evidence_dir` 复用 scheduler `_reject_alias`（目录 `run_dir/selection/point_evidence` 及全部父级 `run_dir/selection`、`run_dir` 拒 symlink/junction/reparse/hardlink）+ `_contained`（解析后位于精确 run_dir 内）；逐条改 `os.scandir` + `is_file(follow_symlinks=False)` + `_reject_alias(entry, require_file=True)`，替换原内联 stat 检查（原检查从未校验目录组件/父级，也未确认 run_dir 内）。`formal_executor` 从 `formal_scheduler` 复用 `_reject_alias`/`_contained`（与既有 `_rebuild_authority` 跨模块私有 import 一致）。
- **缺口 2（accreditation authority preflight）**：`accredit_build` 首步执行**完整 `_rebuild_authority(run_dir, cache_root)`**，以其 replay-verified manifest/plan/fit_states 为唯一事实源——**删掉** raw `manifest.json`/`plan.jsonl` 直读。逐 selection fit 三源一致 fail-closed：scheduler `fit_state` == terminal receipt state == (`failed` iff point-evidence.failed)。preflight + 一致性校验全在写任何诊断**之前**；四个诊断（fit_status/ceiling/leakage/pre_unseal_bundle）只在末尾写，故 preflight/一致性失败时无任何诊断产物。receipt 的 schema/hash/event binding 由 scheduler replay 验证（`_rebuild_authority` 已 hash-bind receipt 到 event 的 `receipt_sha256`）。
- **`formal_scheduler.py` 输出合同未改**；冻结 matrix/plan/`_PLAN_FIELDS`/schema/selection rule/artifact-trace-receipt-bundle version/科学指标/staged 执行**均未改**。

**验证**：**362 非 slow 测试通过**（含 3 个目录 alias 拒绝测试——symlink 在 POSIX / junction 在 Windows 经 `_make_dir_alias`，平台不可创建时明确 skip 不伪造；3 个 preflight attack 测试——真实 materialize+claim+record_fit_succeeded → 篡改 terminal receipt / plan.jsonl / event，`_rebuild_authority` raise 且 fit_status/ceiling/leakage/pre_unseal_bundle 均未生成；legitimate succeeded+failed 路径用既有合成 fixture + mock `_rebuild_authority` 跑通）。focused post-selection authority 测试保持绿。compileall、`git diff --check`、冻结哈希审计（`verify_frozen_hashes` + matrix SHA `fad701…`）全干净。**349-smoke PASSED（2h13min）事实未改、未重跑。**

**未授权/边界**：A-E1 formal 未授权；未进入 A-E3/A-E2、accredit-authorize、test unseal、真实 formal、9d、G4；不自称 formal APPROVE。**完成即提交推送，等 Codex 复审 REVISE。** 报告 `coworker/reports/2026-07-20-study02-g3-point-evidence-revise-claude.md`。
