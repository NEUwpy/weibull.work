...
**Study/02 当前态**：论文主线已收口为一个受控问题：在 Study01 冻结参数网格和相同三参数网络下，仅改变训练/验证损失，`Q_param` 相对本文定义的 `P_equal` 能把目标可靠度寿命点 \(x_{0.95}\) 的误差降低多少，为什么。10-seed 主实验共 200 个 P/Q 对（400 fits）：Q 的 rRMSE 改善 3.00%（95% CI [2.19%,3.79%]），10 个 seed 的 pooled 方向一致。中心机制链为：P 在同一无量纲坐标中采用固定欧氏几何；Q 在当前预测点传播目标误差与动态敏感度；真值点 M95 只是有二维零空间的秩一局部代理；24 单元探索性消融中 M95 rRMSE=0.2005，差于 P 0.1676 与 Q 0.1622，且24/24单元未优于二者；路径平均割线恒等式说明保留动态矩阵自身导数时才与 Q 完全等价；保存预测则显示 Q 的终态参数误差补偿更强。该链排除了“Q 只是一套真值点静态权重”的简单解释，但未分别识别零空间、非线性曲率或优化轨迹的因果贡献。现有预测的事后工程审计进一步发现，Q 的均方收益来自误差再分配：高估和极端尾部下降、低估增加；只有在 \(x_{0.95}\) 用作保证寿命且高估代价更高时，才讨论其潜在工程含义。连续域、Q_direct、中点和 OOD 等后加实验原样保留为探索性归档，不进入当前论文主结论。机制闭环修订已通过独立方法与图文复核，现为 `READY FOR USER REVIEW`；未投稿、未合并 main。入口见 Study/02 [`README.md`](Study/02-study-NN参数估计与分位点目标研究/README.md)、[`docs/研究说明.md`](Study/02-study-NN参数估计与分位点目标研究/docs/研究说明.md) 与 [`paper/论文初稿.md`](Study/02-study-NN参数估计与分位点目标研究/paper/论文初稿.md)。
