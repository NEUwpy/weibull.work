...
**Study/01 当前态**：论文研究的问题是：在三参数 Weibull 小样本 MDM 估计中，偏移量 `delta` 如何影响有限样本估计风险，其合理取值受哪些因素决定，以及在总体参数未知时能否仅依据观测样本实现有效选择。同一 48,000 个样本上，无偏移判据、固定 `delta=0.1` 和样本自适应选择的 pooled J1 分别为 0.9481、0.6304 和 0.5846：正偏移量承担主要的有限样本误差控制，样本自适应选择在其上继续改进精度。正式方法为 Mean-Normalized-MLP：按样本量分别训练 MLP，对排序样本作 `X/mean(X)` 处理，再经训练折内逐位置 StandardScaler 预测 26 点损失曲线并选择偏移量。在 160 组合主设计域（`eta=1000`，`n∈{7,10,15,20}`）内，固定随机种子 42 的 pooled J1=0.5846，较固定 `delta=0.1` 改善 7.2740%（配对 repeat-block bootstrap 95% CI 6.9448%—7.6376%），失败率 0%；160 个设计单元中 125 个改善、35 个退化，单元重采样的 95% 敏感性范围为 6.0367%—8.4603%。另外两个预设种子仅用于初始化敏感性核查。未见 `beta`、WMLE/LSE 外部参照、三个可靠度寿命点和端到端尺度等变检查已完成，其中参数精度收益向可靠度寿命的传递有限。E10 表明较灵活的可观测样本规则仍能低于当前 MLP 的风险，但不是精确 Bayes 风险。E11 在 20 个预设参数单元的 2,000 个确认样本上进一步发现：抽样结果会移动 MDM 经验梯度曲线和搜索交点；固定 `delta=0.1` 时的标准化位置估计与 L6 偏移量的单元内相关中位数为 −0.765，20/20 个单元方向一致，且边界解只占 7.1%。E6 Dimensional-RAW（J1=0.5543）仅作固定尺度敏感性/历史对照。正式方法与机制证据见 `Study/01-study-MDM最小偏移量优化研究/artifacts/formal/E8_mean_normalized_selector/`、`E10_z_only_benchmark/` 和 `E11_profile_mechanism/`，当前正文与附录为 `manuscript/` 下 v1.2；v1.0 和 v1.1 保留未动。真实案例仍后置。
...
| 模块 | 当前状态 |
|------|----------|
| 过程量优化 | MDM 已接入：预测 26 个候选偏移量的损失并选择离散最低点，再调用生产 MDM |
