...
**Study/02 当前态**：主问题已于 2026-08-06 纠偏为“训练与测试覆盖相同参数组合、使用独立 repeats 时，参数损失 P 与目标分位点损失 Q 谁能更准确估计 \(x_{0.95}\)”；fold 1 × 4 个 n × 3 seeds 的 24-fit pilot 已完成，Q pooled rRMSE 约比 P 改善 4.18%，12/12 个 n×seed 单元方向一致，仅作为正式 120-fit 主实验的可行性证据。旧 r4/v3 的 120 fits 数值仍有效，但其整层 gamma/eta 留出实际测量的是对未见 gamma 层级的外推，只作为补充泛化证据。论文研究总入口见 Study/02 `08-PQ-论文研究蓝图与阶段清单.md`；当前论文分支为 `codex/study02-paper-20260806`，未合并 main。
