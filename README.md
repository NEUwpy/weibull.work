...
**Study/01 当前态**：论文主线已收敛为“三参数 Weibull MDM 框架内偏移量 `delta` 是否有更合适的选择，以及怎样根据当前样本实现选择”。正式方法为 Dimensional-RAW-MLP：按样本量分别训练 MLP，以排序原始样本预测 26 点损失曲线并选择偏移量。在 160 组合主设计域（`eta=1000`，`n∈{7,10,15,20}`）内，pooled J1=0.5543，较固定 `delta=0.1` 改善 12.08%，失败率为 0%；结论限于已训练样本量以及训练单位和尺度范围。当前划分已检验未见 `gamma/eta` 水平，但未检验未见 `beta`；per-n specialist 不定义未训练 `n` 的直接应用。旧特征路线、Normalized-RAW、旧 P2/P4/分位点和 NIST 结果已降为历史、候选或 Research 证据。写作前还需补未见 `beta`、WMLE/LSE 同条件参照和 E6 工程寿命分位点；真实案例后置。Study01 活动区已整理为论文所需 `README`、`00`–`04`、证据/代码索引、`Research/` 与 `archive/`；外部中文学术源稿 v0.3 是唯一活动正文。
