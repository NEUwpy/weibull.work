...
**Study/01 当前态**：论文研究的问题是：在三参数 Weibull 小样本 MDM 估计中，偏移量 `delta` 如何影响有限样本估计风险，其合理取值受哪些因素决定，以及在总体参数未知时能否仅依据观测样本实现有效选择。正式方法为 Mean-Normalized-MLP：按样本量分别训练 MLP，对排序样本作 `X/mean(X)` 处理，再经训练折内逐位置 StandardScaler 预测 26 点损失曲线并选择偏移量。在 160 组合主设计域（`eta=1000`，`n∈{7,10,15,20}`）内，固定随机种子 42 的 pooled J1=0.5846，较固定 `delta=0.1` 改善 7.2740%，失败率 0%；另外两个预设种子仅用于初始化敏感性核查。未见 `beta`、WMLE/LSE 外部参照、三个可靠度寿命点和端到端尺度等变检查已完成，其中参数精度收益向可靠度寿命的传递有限。E10 机制证据进一步表明，同一参数条件内的事后低风险偏移量会随样本实现改变，且较灵活的可观测样本规则仍能低于当前 MLP 的风险；该经验参照不是精确 Bayes 风险，其与 L6 的剩余差距不得全部解释为完全信息差。E6 Dimensional-RAW（J1=0.5543）仅作固定尺度敏感性/历史对照。正式方法与机制证据分别见 `Study/01-study-MDM最小偏移量优化研究/artifacts/formal/E8_mean_normalized_selector/` 和 `Study/01-study-MDM最小偏移量优化研究/artifacts/formal/E10_z_only_benchmark/`，当前正文与附录为 `manuscript/` 下 v1.2；v1.0 和 v1.1 保留未动。真实案例仍后置。
