...
> **快照日期**: 2026-08-08 · 以本节为当前状态权威；各分文档的进度表为开发追踪器。
...
**Study/02 当前态**：主问题为“训练与测试覆盖相同参数组合、使用独立 repeats 时，参数损失 P 与目标分位点损失 Q 谁能更准确估计 \(x_{0.95}\)”。正式同分布主实验（S1，120-fit）已完成并获 Codex + mentor 审结：Q 的 pooled rRMSE 约比 P 低 3.1%（模型级配对差 \(\hat\Delta=-0.00171\)，fold×seed 交叉 95% CI 不含 0）；机制（S2）、稳健性与边界（S3）、综合报告与论文级图表（S4）均已完成并获审。当前处于 **S5A 论文初稿预审修订**：R1/R2 已修订并推送，R3 极窄修订（证据边界表述 + 状态/审计文档同步）已完成，待 Codex/Mentor S5A R4 预审；未 APPROVE、未投稿、未合并 main。旧 r4/v3 的 120 fits 数值仍有效，但其整层 gamma/eta 留出实际测量的是对未见 gamma 层级的外推，只作为补充泛化证据。论文研究总入口见 Study/02 `08-PQ-论文研究蓝图与阶段清单.md`；当前论文分支为 `codex/study02-paper-20260806`，未合并 main。
