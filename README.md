...
**Study/02 当前态**：主问题已于 2026-08-06 纠偏为“训练与测试覆盖相同参数组合、使用独立 repeats 时，参数损失 P 与目标分位点损失 Q 谁能更准确估计 \(x_{0.95}\)”；当前先运行 fold 1 × 4 个 n × 3 seeds 的最小 pilot。旧 r4/v3 的 120 fits 数值仍有效，但其整层 gamma/eta 留出实际测量的是对未见 gamma 层级的外推，只作为补充泛化证据：该条件下 P pooled rRMSE 较 Q 约优 8.1%。入口见 Study/02 `README.md` 与 `07-PQ-同分布主实验修正.md`。旧直接标量分位点 D 路线及前置研究 A 继续归档；当前分支尚未合并 main。
