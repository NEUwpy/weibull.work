...
**Study/01 当前态**：论文研究的问题是：在三参数 Weibull 小样本 MDM 估计中，偏移量 `delta` 如何影响有限样本估计风险，其合理取值受哪些因素决定，以及在总体参数未知时能否仅依据观测样本实现有效选择。正式方法为 Mean-Normalized-MLP：按样本量分别训练 MLP，对排序样本作 `X/mean(X)` 处理，再经训练折内逐位置 StandardScaler 预测 26 点损失曲线并选择偏移量。在 160 组合主设计域（`eta=1000`，`n∈{7,10,15,20}`）内，固定随机种子 42 的 pooled J1=0.5846，较固定 `delta=0.1` 改善 7.2740%，失败率 0%；另外两个预设种子仅用于初始化敏感性核查。写作支撑验证已按同一主报告口径重新派生：未见 `beta` 留出（B1，pooled J1=0.5841、改善 7.3473%、失败率 0%；`beta=1.5` 层略差）、WMLE/LSE 同条件外部参照（B2，自身数值仍为 0.7288/0.8725）、`x_0.90/x_0.95/x_0.99` 可靠度寿命派生（B3，Mean-Normalized 相对 RMSE 0.1608/0.2136/0.3753，对 Default 的收益有限），以及 4 个代表条件×3 个尺度的“选择器→生产 MDM”端到端等变检查。E7 候选筛选显示均值与 RMS 归一化近似并列，不声称均值归一化普遍更优。参数引导（plug-in）负向支撑实验与 E1/E2 选择空间保持不变。E6 Dimensional-RAW（J1=0.5543）仅作固定尺度敏感性/历史对照，不再作为论文主方法。per-n specialist 仍不定义未训练 `n` 的直接应用，离散留出也不支持连续参数空间的普遍泛化。正式 E8 证据见 `Study/01-study-MDM最小偏移量优化研究/artifacts/formal/E8_mean_normalized_selector/`，当前正文与附录为 `manuscript/` 下 v1.2；v1.0 和 v1.1 保留未动。真实案例仍后置。
