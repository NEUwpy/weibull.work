...
**Study/02 当前态**：前置研究 A 精简复现路线 P0–P6 已完成，19/19 问有证据覆盖（17 answered + A4/A8 partial）。结果不支持“NN 普遍优于传统方法”：NN 在部分参数层有优势、跨 seed 稳定并满足尺度/平移等变，但强异常值显著增大误差；NIST 真实数据 CDF 拟合有竞争力，γ 支持边界却经常违规；core conformal 区间覆盖接近标称值，boundary 域明显失效。旧 formal 引擎与 test namespaces 永久只读/sealed。下一步是否进入 Study/02 分位点主研究由用户另行决定；正式成果见 Study/02 `06-A-前置研究报告.md`，逐题证据见 `05-A-证据索引.md`。
