...
> **快照日期**: 2026-08-19 · 以本节为当前状态权威；各分文档的进度表为开发追踪器。
...
**Study/01 当前态**：论文主线已收敛为“三参数 Weibull MDM 框架内偏移量 `delta` 是否有更合适的选择，以及怎样根据当前样本实现选择”。正式方法为 Mean-Normalized-MLP：按样本量分别训练 MLP，对排序样本作 `X/mean(X)` 处理，再经训练折内逐位置 StandardScaler 预测 26 点损失曲线并选择偏移量。在 160 组合主设计域（`eta=1000`，`n∈{7,10,15,20}`）内，pooled J1=0.5850，较固定 `delta=0.1` 改善 7.2038%，三 seed 标准差 0.0004，失败率 0%；分 `n` J1 为 0.7020/0.6074/0.5284/0.4774。写作支撑验证已按正式方法重新派生：未见 `beta` 留出（B1，pooled J1=0.5829、改善 7.536%、失败率 0%；`beta=1.5` 层略差）、WMLE/LSE 同条件外部参照（B2，自身数值仍为 0.7288/0.8725）、`x_0.90/x_0.95/x_0.99` 可靠度寿命派生（B3，Mean-Normalized 相对 RMSE 0.1609/0.2134/0.3744，对 Default 的收益有限），以及 4 个代表条件×3 个尺度的“选择器→生产 MDM”端到端等变检查。E7 候选筛选显示均值与 RMS 归一化近似并列，不声称均值归一化普遍更优。参数引导（plug-in）负向支撑实验与 E1/E2 选择空间保持不变。E6 Dimensional-RAW（J1=0.5543）仅作固定尺度敏感性/历史对照，不再作为论文主方法。per-n specialist 仍不定义未训练 `n` 的直接应用，离散留出也不支持连续参数空间的普遍泛化。正式 E8 证据见 `Study/01-study-MDM最小偏移量优化研究/artifacts/formal/E8_mean_normalized_selector/`，当前正文与附录为 `manuscript/` 下 v1.1；v1.0 及更早版本保留未动。真实案例仍后置。
