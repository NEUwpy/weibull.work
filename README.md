...
> **快照日期**: 2026-08-28 · 以本节为当前状态权威；各分文档的进度表为开发追踪器。
...
**Study/02 当前态**：论文研究参数恢复损失与下游可靠度寿命点之间的评价尺度错配，以 $x_{0.95}$ 定义任务导向损失 Q，并以参数损失 P 构造 QCP 可行域。正式研究域为 Study01 广参数域：$\beta\in\{1.5,2,2.5,3,3.5,4,4.5,5\}$、$\gamma/\eta\in\{0.10,0.25,0.50,0.75,1.00\}$，$\eta=1000$，$n\in\{7,10,15,20\}$。P、Q、QCP 在相同数据、拆分、网络和 `600/60` 预算下形成 200 个配对模型单元。Q 在训练目标 $x_{0.95}$ 上相对 P 改善 2.069%，但在 $x_{0.90}$ 和 $x_{0.99}$ 上分别退化 36.411% 和 63.114%，直接暴露单点目标监督造成的跨寿命点失真。QCP 消除 Q 相对 P 的 97.8% 超额参数补偿；相对 P 在三个寿命点分别改善 2.353%、3.599% 和 5.981%，相对 Q 分别改善 28.417%、1.562% 和 42.360%。当前主贡献为“Q 对齐目标寿命点，P 约束恢复跨寿命点一致性”。当前工作稿为 [`paper/论文初稿-v2.1-正面叙述精简版.md`](Study/02-study-NN参数估计与分位点目标研究/paper/论文初稿-v2.1-正面叙述精简版.md)。

**Study/01 当前态**：论文研究的问题是：在三参数 Weibull 小样本 MDM 估计中，偏移量 `delta` 如何影响有限样本估计风险，其合理取值受哪些因素决定，以及在总体参数未知时能否仅依据观测样本实现有效选择。图 1 以组合内重复抽样离散程度确认了正偏移量相对于无偏移判据的稳定性作用，后续主比较以固定 `delta=0.1` 为经验基准。E13 进一步表明，固定宽度 `beta` 参数域由 `[1.50,2.50]` 平移到 `[4.00,5.00]` 时，最佳统一偏移量由 0.18 移至 0.04；统一值因而是给定参数域下的汇总折中，`delta=0.1` 可作为宽混合参数域中的稳健默认值，但不是脱离参数域的唯一最优常数。正式方法为 Mean-Normalized-MLP：按样本量分别训练 MLP，对排序样本作 `X/mean(X)` 处理，再经训练折内逐位置 StandardScaler 预测 26 点损失曲线并选择偏移量。在同一 48,000 个测试样本上，固定随机种子 42 的 pooled J1=0.5846，较固定 `delta=0.1` 的 0.6304 改善 7.2740%（配对 repeat-block bootstrap 95% CI 6.9448%—7.6376%），三个参数的标准化 Bias 绝对值、SD 和 RMSE 均下降，失败率 0%；160 个设计单元中 125 个改善、35 个退化。另外两个预设种子仅用于初始化敏感性核查。未见 `beta`、WMLE/LSE 外部参照、三个可靠度寿命点和端到端尺度等变检查已完成，其中参数精度收益向可靠度寿命的传递有限。E10 表明较灵活的可观测样本规则仍能低于当前 MLP 的风险，但不是精确 Bayes 风险。E11 在 20 个预设参数单元的 2,000 个确认样本上进一步发现：抽样结果会移动 MDM 经验梯度曲线和搜索交点；固定 `delta=0.1` 时的标准化位置估计与 L6 偏移量的单元内相关中位数为 −0.765，20/20 个单元方向一致，且边界解只占 7.1%。E6 Dimensional-RAW（J1=0.5543）仅作固定尺度敏感性/历史对照。正式方法与机制证据见 `Study/01-study-MDM最小偏移量优化研究/artifacts/formal/E8_mean_normalized_selector/`、`E10_z_only_benchmark/`、`E11_profile_mechanism/` 和 `E13_beta_domain_sensitivity/`；当前正文为 v1.5，附录为 v1.4，历史版本保存在 `manuscript/shelve/`。真实案例仍后置。
