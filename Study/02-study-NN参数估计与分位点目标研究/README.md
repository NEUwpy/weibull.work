...
> 创建日期：2026-07-11
> 当前修订：2026-08-10（S5B 新增 880 fits 与论文大修完成，等待独立复审；
> 论文不得提前标为 APPROVE；未投稿、未合并 main）
...
冻结网格的确认性 10-seed 对照表明 Q_param 相对 P_equal 改善 3.00% [2.19%,3.79%]；
但连续域内仅改善 0.33%且区间跨 0，Q_direct 改善 0.74%，中点和 gamma 留出均由 P 更优。
因此当前科学问题已从“Q 是否更好”收口为“目标对齐在何种数据覆盖与表示下改善多少”。
...
- **主协议拆分**：每组合 300 个 repeats 按 `repeat_id % 5` 五折（test 60 / val 60 / train 180），
  ...
  OOD 补充，不作主推断
- 主实验训练随机种子扩为 `42, 2026, 3407, 17, 73, 314, 2718, 4099, 8128, 12011`；隐藏层 `256–128–64`、ReLU、Adam、batch 256、
...
| `13-PQ-综合科学报告.md` | **S5B 当前权威科学总结**（网格、连续、中点、OOD 四合同与结论边界） |
| `14-PQ-论文初稿.md` | **S5B 论文修订稿**（IMRaD，中英摘要 + 15 条核验文献；嵌入 fig1–fig4） |
| `15-PQ-补充材料.md` | **S5B 补充材料**（10 seeds、三路线、统计推断、结果与证据） |
| `16-PQ-引用与完整性审计.md` | **S5B 引用与完整性审计**（claim→evidence、引用、图、最终门） |
...
| `18-PQ-S5B审稿意见逐条回复.md` | 模拟审稿 R1–R7 逐条回复与证据定位 |
...
10. `14-PQ-论文初稿.md`：S5B 论文修订稿（与 `15`、`16`、`18` 配套）。
