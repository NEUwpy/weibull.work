# 三参数 Weibull 神经估计中的目标对齐：动态目标损失与辅助参数约束

> 论文工作稿 v1.2 · 受控仿真研究 · 2026-08-27
> 状态：`QP RESULT INTEGRATED — DRAFT FOR USER REVIEW — NOT SUBMITTED`
...
真值点静态参数权重”这一简单解释，但不将未观测的训练轨迹写成已证因果。在此基础上，本文进一步
考察辅助参数项 `QP`：训练损失为 \(L_Q+\lambda_PL_P\)，但 checkpoint 选择和最终成败仍只依据
\(x_{0.95}\) 误差。训练预算和 \(\lambda_P\) 均先在测试集封存的验证筛选中确定，再进行独立
10-seed 正式确认。P、Q、QP 的测试 rRMSE 分别为 0.1661、0.1611 和 0.1595；QP 相对 Q
改善 0.984%，相对 P 改善 3.955%，两个设计级 bootstrap 区间均完全低于 0，且 10/10 个
种子方向一致。QP 将 Q 的平均参数损失从 78.998 降至 0.0584，将参数补偿指数从 0.915 降至
0.381，同时保持最低目标误差，支持“参数项减少目标等价解中的过度参数补偿”这一解释。
附加探索性工程审计
...
本文不主张 Q 的内部参数更准确，也不把 QP 写成对 Q 的理论必然改进：普通加权和允许目标误差与
参数误差交换，验证筛选中 \(\lambda_P=0.1\) 即劣于纯 Q。结论不外推到其他目标、参数域或部署分布。

**关键词**：三参数 Weibull；可靠度寿命点；目标对齐；动态目标敏感度；辅助参数约束；神经网络
...
been causally identified. We then evaluated an auxiliary-parameter route, QP, trained with
\(L_Q+\lambda_PL_P\) while selecting checkpoints and judging success solely by \(x_{0.95}\) error. The
training budget and \(\lambda_P\) were selected using validation data with test metrics sealed, followed by
an independent ten-seed confirmation. Test rRMSE was 0.1661 for P, 0.1611 for Q, and 0.1595 for QP.
QP improved over Q by 0.984% and over P by 3.955%; both design-bootstrap intervals lay entirely below
zero and all ten seed-level effects favored QP. QP reduced mean parameter loss from 78.998 under Q to
0.0584 and the parameter-cancellation index from 0.915 to 0.381 while retaining the lowest target error.
An additional exploratory engineering audit showed a trade-off rather than
...
ablation result, not the full causal training dynamics. A weighted QP loss is also not theoretically guaranteed
to preserve the pure-Q optimum: the validation screen included a \(\lambda_P=0.1\) setting that performed
worse than pure Q. We do not claim generalization to other targets, domains, or deployment distributions.
...
5. 在测试指标封存的验证筛选后，用 10-seed 正式实验检验参数损失作为辅助项能否进一步改善目标精度，并诊断参数补偿是否下降。
...
\[
L_{QP}=L_Q+\lambda_PL_P.
\]

QP 与 P/Q 使用相同三参数输出和合法解码。P 只作为训练辅助项；QP 的 early stopping、checkpoint
选择和最终成功标准全部使用验证集或测试集 Q，不使用混合目标。该写法是软辅助正则，而不是硬约束：
它允许 Q 与 P 之间发生交换，不能保证任意 \(\lambda_P\) 都不劣于纯 Q。

超参数筛选完全封存测试指标。资源门在 seed 42、fold 1/3 和四个 \(n\) 上比较 `300/20` 与
`600/60` 的最大 epoch/patience；延长预算只使 Q 的验证 rRMSE 改善 0.237%，低于预设 0.5%
成本—收益门，因此保留 `300/20`。随后在 8 个相同验证单元上筛选
\(\lambda_P\in\{0,10^{-6},10^{-5},10^{-4},10^{-3},10^{-2},0.1,1\}\)，选择验证 Q
最低的 \(\lambda_P=1\)。筛选共 96 fits，manifest 记录测试指标访问 0 次。

### 2.3 配对与重复性

训练种子为 42、2026、3407、17、73、314、2718、4099、8128、12011。每个
`(n,fold,seed)` 内，P/Q 共享训练、验证、测试行、scaler、初始化、batch 顺序、网络结构和
训练预算，形成 200 个配对单元（400 fits）。冻结 \(\lambda_P=1\) 后，QP 在相同 200 个单元
新增 200 fits，并逐单元核对上述配对标识。10 seeds 的职责是确认效应并估计训练随机性；
...
QP 正式确认预设两个共同主要对比：QP−Q 与 QP−P 的模型单元 MSE 差均须小于 0，且设计级
bootstrap 95% 区间完全低于 0；同时报告 200 单元和 10 seeds 的方向计数。QP 的参数损失和
参数补偿指数只用于机制诊断，不参与成败判定。
...
验证筛选选出的 QP 在独立测试中的 rRMSE 为 0.159491，低于 Q 的 0.161075 和 P 的 0.166059。
QP 相对 Q 改善 0.984%，模型单元 MSE 差为 −0.000508，设计级 bootstrap 95% 区间为
[−0.000737,−0.000290]；158/200 单元、10/10 seeds 方向有利。QP 相对 P 改善 3.955%，
MSE 差为 −0.002138，95% 区间为 [−0.002464,−0.001828]；199/200 单元、10/10 seeds
方向有利。两个共同主要判据均通过。
...
*图1b注：面板 a、b 只使用验证集；面板 c 才是冻结配置后的独立测试确认。辅助权重的收益非单调，
\(\lambda_P=0.1\) 的验证 Q 劣于纯 Q，因此不能把 QP 写成理论上的必然改进。*
...
### 3.3 QP 如何改变参数补偿与目标误差

QP 的机制诊断与“减少过度参数补偿”一致。纯 Q 的平均参数损失为 78.998，参数补偿指数为 0.915；
QP 分别降至 0.0584 和 0.381，接近 P 的 0.0546 和 0.351。与此同时，QP 的目标 rRMSE 最低。
这说明当前 \(\lambda_P=1\) 倾向于从 Q 的低目标误差区域中选择参数补偿较小的解，但由于使用的
仍是加权和，该机制是经验支持的倾向，不是“严格沿等 Q 谷底移动”的数学保证。

QP 相对 Q 的 MAE 从 0.11225 降至 0.11077，绝对误差 95% 分位点从 0.30780 降至 0.30254，
global CVaR90 从 0.36291 降至 0.35820；低估超过 10% 从 28.70% 降至 28.04%，高估超过
10% 基本不变。QP 相对 P 的逐样本绝对误差胜率只有 47.0%，所以它胜过 P 不是因为多数样本都
更接近真值，而是因为较大的平方误差和尾部控制得更好。
...
P/Q 的 400 fits 与 QP 的 200 fits 均完成，0 NaN、0 非有限预测、0 支持域违规。三路线的样本行、scaler、初始化、
...
### 4.4 为什么QP改善了，但仍需要区分混合损失与约束优化

QP 的正式结果说明，在当前合同下，经过验证选择的参数辅助强度可以同时改善纯 P 与纯 Q。然而，
普通加权和最小化的是 \(Q+\lambda P\)，优化器可以用少量 Q 退化换取较大的 P 改善。因此本文只把
QP 解释为经实验确认的软辅助路线，不声称其具有“Q绝不退化”的理论性质。若要严格表达“Q为主、
P只作约束”，更合适的问题是
...
并由乘子或增广拉格朗日方法只在参数约束被违反时激活 P 梯度。该硬约束路线属于下一项独立研究，
不以尚未完成的结果改变本文 QP 结论。

### 4.5 潜在工程含义不是“全面更准”，而是“降低哪种错误”
...
3.00%（95% CI [2.19%,3.79%]）。进一步加入经验证选择的辅助参数项后，QP 的 rRMSE 为
0.159491，相对 Q 改善 0.984%、相对 P 改善 3.955%，两个正式对比的设计级 bootstrap 区间
均完全低于 0，且 10 个训练 seed 的方向一致。与这些优势相容的机制证据是：
P 在归一化参数坐标中使用固定误差规则，而 Q 在当前预测点按 B5 的实际敏感度更新三个参数方向。
真值点静态矩阵 M95 在 24 单元探索性消融中未能替代 Q；它虽然改善了局部近似，但这个近似遗漏的
额外目标误差更大，真实 B5 因而变差；QP 又显著减少了纯 Q 的参数补偿，同时取得最低目标误差。
探索性工程审计进一步表明，均方误差收益主要来自压低寿命高估侧和极端
尾部，并以更多低估为代价。因此，当工程任务只评价一个已知可靠度寿命点，且高估具有更高
工程代价时，当前结果支持 `Q_param` 相对本文定义的 `P_equal` 更符合该风险方向。该结论
只针对当前冻结合同。QP 是软辅助加权和，不具有不劣于 Q 的理论保证，也不延伸为普遍优越性或参数估计替代论。
...
QP 验证筛选与正式确认分别见 `artifacts/pq_regularized_pilot/` 和
`artifacts/pq_regularized_confirm/`，对应合同为 `protocols/12-*` 与 `protocols/13-*`。
...
*本版本已纳入 QP 10-seed 正式结果，当前可交付用户审阅；此状态不等于期刊同行评议或录用。*
