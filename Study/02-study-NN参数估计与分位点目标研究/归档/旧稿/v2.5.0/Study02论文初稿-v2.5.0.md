...
不同真值点的敏感度方向不同，将损失对参数网格求和可以得到高于一的秩；但条件网络输出的是随输入样本变化的参数，而非所有样本共用的一组三参数。它可能对不同输入沿各自的近等寿命点曲面发生不同偏移，因此跨参数点平均不会自动提供每个输入处缺失的参数监督。共享网络权重、容量和正则化又限制这些偏移能否同时实现。局部几何说明了可能的补偿方向，终态参数分布与跨点误差用来判断实际训练是否利用了这些方向，不能仅凭输出空间的秩一结论推定网络权重空间的退化程度。
...
QCP 的 200 个选定模型检查点均满足验证集平均参数约束。其测试平均 $L_P$ 为 0.0552，补偿指数为 0.345，均接近 P。按 $(C_Q-C_{\mathrm{QCP}})/(C_Q-C_P)$ 计算，QCP 使 Q 相对 P 的超额补偿减少 97.8%。这是参数约束达到预期作用的终态诊断：参数误差收缩与补偿指标回落相伴。更关键的问题是，这种收缩是否以牺牲目标寿命点精度为代价。

参数恢复伴随跨寿命点误差下降。QCP 在目标点较 Q 再改善 1.56%（95% CI：1.24% 至 1.90%），在两个非目标点则分别改善 28.42% 和 42.36%。与 P 相比，三个寿命点的总体 RMSRE 均小幅降低（表1）。QCP 的主要作用是保留单点目标对齐，并把其余寿命点恢复到接近 P 的水平；其目标点增益保持在几个百分点的量级。
...
为定位收益来源，定义每个真值单元的贡献 $\Delta_c=\mathrm{MSE}_{P,c}-\mathrm{MSE}_{\mathrm{QCP},c}$。在 $x_{0.95}$ 上，将 160 个单元按 $\Delta_c$ 从大到小排列，前 5 个单元的改善合计达到全部净改善的 102.6%；其余 155 个单元合计略有抵消（图3）。按 P 的单元 RMSRE 四等分后，误差最高四分组的平均 $\Delta_c$ 为 0.00988，中间两组分别为 −0.000706 和 −0.00159，最低组接近零。QCP 对 P 的总体收益主要由少数高误差单元贡献，而不是随单元难度普遍增加。分组使用同一批 P 结果，仅用于描述收益构成，不作为基线难度与改善之间的独立因果检验。
...
对全部测试预测的参数分布检查表明，Q 的 $\hat\beta$ 中位数为 20.39、$\hat\eta$ 中位数为 67.00，而 P 对应为 3.03 和 996.92。Q 表现为形状参数增大、尺度显著缩小及位置估计整体上移。定义相对边界距离 $d=(\min(X)-\hat\gamma)/\min(X)$，Q 仅有 0.014% 的预测满足 $d\le10^{-3}$，1.56% 满足 $d\le10^{-2}$。因此，当前退化主要体现为参数组合的系统性改变，而非位置参数普遍贴住解码上界；完整分布及边界诊断见附录 B.6。

若预测器只用于 $x_{0.95}$，参数补偿本身可以与目标精度并存；当同一组参数还要支持其他寿命点或分布解释时，补偿便成为实质问题。Q 在 $x_{0.90}$ 与 $x_{0.99}$ 上的 RMSRE 分别比 P 高 36.41% 和 63.11%，说明目标点精度没有传递到整组三参数预测。Q 因而承担机制中间路线的作用：它验证任务对齐能够改善目标点，也暴露单点监督留下的自由度。
...
这条比较说明了任务对齐的收益、单点监督的代价以及参数约束的修复作用。相应认识适用于当前固定尺度、小样本设计域及各自的验证选点流程；训练损失与选点规则的独立贡献、以及参数约束相对多点监督的优势，需要由针对性对照进一步区分。

## 数据与代码可用性

支持本研究的仿真协议、训练与派生分析代码、模型单元汇总、配对预测和校验清单将随论文公开。正文数值来自共同预算下的 P、Q、QCP 配对分析，以及基于同一批预测得到的跨寿命点、区域分布和偏差—方差结果。复算所需的数据结构与文件对应关系见[附录 E](Study02论文附录-v2.5.0.md)；早期训练预算的结果单列于附录 D。
...
[附录 A–E：实验与推断细节、完整结果、敏感度推导、历史预算分析及复算索引](Study02论文附录-v2.5.0.md)。
