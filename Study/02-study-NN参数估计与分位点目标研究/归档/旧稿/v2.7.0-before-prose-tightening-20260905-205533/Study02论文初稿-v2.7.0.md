...
*图1　网络结构与三种监督原理。A显示排序样本、在训练集拟合的标准化、三层全连接ReLU隐藏层、三输出解码和Weibull寿命点计算。圆点仅为神经元示意，实际层宽为n–256–128–64–3；原始样本最小值单独进入位置参数解码。B的网络缩略图均代表A中的同一结构，各路线独立训练。P将三个归一化参数误差平方相加后取样本均值（Σ表示图中三项之和）；Q经同一组预测参数计算目标寿命点，再与真寿命点比较；QCP将小批量目标损失与参数约束惩罚组合。Ψ为正文定义的增广拉格朗日惩罚，依赖约束违反量、乘子μ和系数ρ，μ逐轮更新；τ来自早期P参考模型的验证损失。虚线表示损失梯度回传或阈值依赖。底部曲线和散点均为规则示意，不是实验测量：P/Q分别选取最低验证参数/目标损失，QCP在验证平均参数损失不超过τ的检查点中选择目标损失最低者，空心圈表示选中点。验证可行性不提供逐条预测误差上界。*
...
固定 $\eta$ 下，该网格的真实 $x_{0.95}$ 仍为 238.05–1552.09。本文采用相对误差，使各寿命水平按比例误差比较。尺度标签始终为1000，P可通过学习常量降低尺度误差，而Q允许牺牲尺度恢复以减小寿命点误差。因此，本设计中的参数补偿也受到固定尺度标签的影响，不能直接外推到尺度变化的估计任务。结论对应固定物理尺度下的参数网格与小样本估计。
...
当约束松弛且乘子为零时，参数项不参与更新；约束受到违反时，其作用由乘子与违反程度调节。模型检查点选择先检查验证集平均 $L_P\le\tau_j$，再从可行训练轮次中选出验证 $L_Q$ 最小者。训练阶段通过小批量参数损失施加约束惩罚；检查点可行性则以验证集平均参数损失判断。二者都不提供单条预测的误差上界。
...
*图2　A固定 $\gamma=100$、真值 $(\beta,\eta)=(1.5,1000)$，展示二维输出切片；虚线为相同 $x_{0.95}$ 的参数轨迹，灰线为绝对相对寿命点误差等高线。B直接展示200个QCP选定检查点的验证集平均参数损失与目标点误差；横轴为平均 $L_P/\tau_j$，虚线为可行性边界。该约束不是单条预测的误差上界。C使用附录C.1的精确对称分解；每条路线480,000条预测，标记为中位数、粗线为四分位范围、细线为5%–95%范围，均为描述性分布范围。D每点为一个模型单元：先逐样本计算分量绝对值之和与相加后的绝对误差，再分别取均值；虚线表示无抵消。插图用相同横纵比例放大P/QCP点簇，Q点簇标出样本量。每路线200个模型单元，底层测试样本在训练种子间共享。*
...
为定位收益来源，定义每个真值单元的贡献 $\Delta_c=\mathrm{MSE}_{P,c}-\mathrm{MSE}_{\mathrm{QCP},c}$。在 $x_{0.95}$ 上，将 160 个单元按 $\Delta_c$ 从大到小排列，前 5 个单元的改善合计达到全部净改善的 102.6%；其余 155 个单元合计略有抵消（图4）。按 P 的单元误差分组后，正收益主要来自误差最高的四分组，中间两组平均略偏向 P。由此可见，QCP 对 P 的总体收益集中于少数高误差单元，而非各区域普遍改善。前五个单元均位于低形状、低位置比区域（附录图B4）；完整单元身份、分组数值及事后分解的解释范围见附录B.7。
