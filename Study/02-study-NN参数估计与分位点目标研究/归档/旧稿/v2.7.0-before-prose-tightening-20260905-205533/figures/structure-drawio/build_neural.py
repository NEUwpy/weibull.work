...
def build(en=False):
 x=E.Element('mxfile',host='app.diagrams.net',agent='Codex',version='26.0.9');d=E.SubElement(x,'diagram',id='study02',name='Network and supervision');g=E.SubElement(d,'mxGraphModel',page='1',pageWidth='1900',pageHeight='1535',grid='1',gridSize='10');r=E.SubElement(g,'root');E.SubElement(r,'mxCell',id='0');E.SubElement(r,'mxCell',id='1',parent='0');geo={}
 ...
 text('nnnote',tr('圆点示意，层宽以标注为准','Representative neurons; widths shown above'),265,408,720,36,23)
 ...
 text('forwardnote',tr('固定网络推理：只输入寿命样本；真参数用于仿真监督。','Fixed-network inference uses lifetime samples only; truth supplies simulation labels.'),30,472,1830,45,25)
 text('titleB',tr('(b) 相同网络，三种监督原理；各路线独立训练','(b) Same architecture, three supervision rules; independent fits'),25,530,1850,60,38,True)
 for route,base,color,fill,title in [('P',30,'#737373','#F0F0F0',tr('P  逐参数误差监督','P  Parameter-error supervision')),('Q',660,'#0072B2','#E5F1F8',tr('Q  单寿命点监督','Q  Single-life-point supervision')),('QCP',1290,'#178064','#E5F2EC',tr('QCP  目标监督＋参数约束','QCP  Constrained target loss'))]:
  ...
  if route=='P':
   ...
   text('Qmeaning',tr('参数误差可在同一<br>目标点相互抵消','Parameter errors can<br>cancel at one target'),base+10,1060,330,80,24);loss='Q_loss'
  else:
  ...
  text(route+'gradlabel',tr('反向传播 · 更新 θ','Backpropagate · update θ'),base+125,1165,430,40,23,False,color)
 ...
 text('footer',tr('实线：前向计算；虚线：梯度回传或阈值依赖。小图为选点规则示意；QCP 可行性针对验证集平均参数损失。','Solid: forward computation. Dashed: gradients or thresholds. QCP feasibility uses mean validation parameter loss.'),25,1480,1850,45,23)
