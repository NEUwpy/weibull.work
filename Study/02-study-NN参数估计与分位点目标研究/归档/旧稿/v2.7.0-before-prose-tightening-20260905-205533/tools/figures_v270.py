...
def mechanism(blocks,models):
    ...
    def finish(fig,name):
        ...
        ax.text(.04,.95,'200/200 检查点可行\n不表示每条预测满足误差上界',transform=ax.transAxes,va='top',fontsize=8)
...
def heterogeneity(cells):
    ...
    def finish(fig,name):
        ax=fig.axes[1]
        ax.text(.29,.09,'前五单元的位置见附录图 B4\n主要位于低形状、低位置比区域',transform=ax.transAxes,fontsize=8)
        save(fig,name)
...
def appendix(summary):
    ...
    axs[0].set(xlabel='可靠度 R',ylabel='寿命点 RMSRE（%）',title='A  较宽可靠度范围（描述性）')
...
def historical():
    ...
    def save_history(fig,name,out):
        ...
        translations.update({
            ...
            '(a) Q shifts the error distribution toward\nunderestimation (exploratory)':'A  有符号误差分布（探索性）',
            '(b) Q is slightly worse at moderate errors\nbut better in the far tail (exploratory)':'B  绝对误差的超越比例（探索性）',
            '(c) Under a guaranteed-life interpretation, Q\nreduces overestimation but increases underestimation':'C  高估与低估的再分配（探索性）',
