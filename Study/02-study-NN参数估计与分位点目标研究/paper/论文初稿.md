# 三参数 Weibull 神经估计中的目标对齐：敏感度几何、参数误差补偿与目标误差再分配

> 论文工作稿 · 受控仿真研究 · 2026-08-10
> 状态：`INDEPENDENT REVIEW PASSED — READY FOR USER REVIEW — NOT SUBMITTED`
...
配对单元有利于 Q。在与 P 相同的无量纲参数坐标中，`P_equal` 是固定欧氏几何，而 Q 损失
通过当前预测点的目标误差与敏感度形成动态几何；冻结网格上真值敏感度范数变化 5.74 倍。
作为探索性消融，真值点秩一局部代理 M95 在 24 个匹配单元中的 rRMSE 为 0.2005，差于
P 的 0.1676 和 Q 的 0.1622，且 24/24 单元均未优于 P 或 Q。这排除了“Q 的作用只是一套
真值点静态参数权重”这一简单解释，但不分别识别零空间与非线性曲率的因果贡献。保存预测的精确分解进一步显示，Q 的
参数误差分量抵消更强（P 0.351，Q 0.915；200/200 配对单元均为 Q>P）。附加探索性工程审计
...
编码高估的额外成本。损失几何与最终结果结构均有直接证据，但完整训练动力学未被写成已证因果。
...
loss level and in the same dimensionless coordinates used by P, `P_equal` defines a fixed Euclidean
geometry, whereas Q induces a dynamic geometry through the target error and sensitivity at the current
prediction. The truth-point sensitivity norm varied 5.74-fold over the frozen grid. In an exploratory
24-cell ablation, the truth-point rank-one local surrogate M95 had an rRMSE of 0.2005, compared with
0.1676 for P and 0.1622 for Q, and did not outperform either route in any cell. This rules out the simple
account that Q acts only as a static truth-point parameter weighting under the frozen contract, without
separately identifying the causal contributions of null-space drift and nonlinear curvature. An exact decomposition
of saved predictions showed stronger parameter-error cancellation under Q (0.915 vs 0.351; Q>P in
all 200 paired units). An additional exploratory engineering audit showed a trade-off rather than
...
remains symmetric and does not explicitly encode an asymmetric cost. These results establish loss geometry and the fitted result-space
pattern, not the full causal training dynamics. We do not claim that Q recovers individual parameters
...
2. 从 Weibull 目标函数的梯度与局部曲率推导 Q 如何形成随参数区域变化的目标敏感度几何；
3. 证明真值点秩一矩阵只是 Q 的局部二次代理，并以 24 单元 M95 失败消融检验“Q 只是静态加权”的解释；
4. 用路径平均割线恒等式连接局部几何与有限预测误差，再以逐样本精确分解说明终态参数补偿；
5. 探索均方误差收益对应何种误差再分配，并同时呈现高估下降与低估增加；
6. 将多 seed、配对 bootstrap 和证据完整性明确定位为可信性验证，而非新的科学问题。
...
并记 \(e(u)=(\hat x_R-x_R)/x_R\)。在该共同坐标中，`P_equal` 定义固定欧氏几何：
...
\(H_Q(0)\) 包含参数方向之间的交叉项，但秩仅为一：沿 \(s_0^Tu=0\) 的二维零空间，
真值点二阶近似不惩罚参数组合变化。冻结 40 点上 \(\|s_0\|\) 为 0.766–4.393，方向最大相差
20.3°。因此，不仅一组固定对角权重不能逐点复制 Q，即使一组固定完整矩阵也不能同时复制全域内
不断改变的幅度和方向。Q 不是额外学习三个全局权重，而是由目标公式和链式法则在当前预测位置
生成局部几何；传播到网络更新时还要乘解码器与共享网络 Jacobian，并受优化器影响。
...
M95 随每条训练标签的真值参数点变化，但对当前预测位置静态；它只是 Q 在 \(u=0\) 处的二阶
局部项。取任一非零 \(v\in\ker(s_0^T)\)，都有 \(L_{M95}(\lambda v)=0\)。只要目标沿该方向
存在非零高阶变化，有限 \(\lambda\) 下便可能有 \(e(\lambda v)\ne0\)，从而 \(L_Q>0\)。这给出
静态秩一代理存在零空间盲区的构造性反例，但不表示零空间必然单独造成实际训练差距。

对任意有限扰动，微积分基本定理给出路径平均敏感度

$$
\bar s(u)=\int_0^1g(tu)\,dt,\qquad
e(u)=\bar s(u)^Tu,\qquad
L_Q=u^T[\bar s(u)\bar s(u)^T]u.
$$

这是一条精确的割线恒等式。只有在求导时保留 \(\bar s(u)\) 对 \(u\) 的依赖，动态矩阵表达才与
Q 的前向值和梯度恒等；若把矩阵 `detach` 后再优化，它就是另一种 surrogate。由此，Q 可以看成
随当前有限误差路径变化的目标度量，而 M95 只是其真值点静态局部近似。
...
区域汇总时，\(\|s_0\|\) 与 Q 的绝对 rRMSE 优势 Pearson \(r\) 分别为 0.95（八个 bin）和
0.86（五个 bin）。这是探索性区域关联，不是必要或充分条件的因果检验。

为检验“Q 只是把 P 换成一套更聪明的静态参数权重”，本文纳入一个主结果完成后另行预设的
探索性消融。M95 使用每条训练标签真值点的完整 \(s_0s_0^T\)（包括交叉项），但不随当前预测
位置更新。在 3 seeds × 2 folds × 4 n 的 24 个 P/M95/Q 匹配单元上，rRMSE 分别为
P 0.1676、M95 0.2005、Q 0.1622；M95 相对 P 变差 19.61%，24/24 单元均未优于 P 或 Q。
该小型消融不报告置信区间或显著性。它说明，在当前冻结网络、网格和优化合同中，真值点静态秩一
局部代理不足以替代 Q。结果与零空间盲区和离开真值后的非线性曲率共同造成差距的解释相容，但没有
分别识别二者贡献，也不否定其他目标感知参数度量。

对这 24 个单元的 held-out 终态误差再作精确 Taylor 分解，M95−P 的真实 B5 MSE
差为 +0.01210。其中真值点局部项为 −0.00976，但局部项与非线性余项的交叉项为
+0.00415，余项平方为 +0.01770，三项之和精确回到 +0.01210。因而 M95 确实降低了它所
表示的局部代理，但未受约束的有限误差几何使真实 B5 误差更大。
...
![图二：固定参数几何、动态目标敏感度、静态矩阵消融与终态补偿](../figures/pq-paper/fig2_mechanism.png)

图 2a 的 B10/B5/B1 曲线只是在冻结真值网格上对解析敏感度的计算，不是新的多目标训练；
图 2b 的柱为 24 个单元等权聚合的 rRMSE，黑点为 3 个 seed 各自的 8 单元聚合值。图 2b、d
使用 24 个匹配单元，仅作探索性消融、不给置信区间；图 2c 使用完整 200 个 P/Q
配对单元。所有终态几何量都不解释为 SGD 轨迹的因果分解。
...
并通过 \(ss^\mathsf T\) 表示参数方向之间对最终目标的共同作用。它既避免人工拍定一组全域固定权重，
也允许“参数单项不够准、目标组合仍然准”的补偿解。若为 P 人工构造随参数点变化的完整局部度量，
实际上就是把目标 Jacobian 重新写回参数损失，接近 Q 的目标几何而不再是任务无关参数精度。
M95 消融进一步说明，即使在真值点写入完整交叉矩阵，也没有复制 Q：局部矩阵的二维零空间与 Q
沿有限误差路径变化的敏感度都被静态近似丢失。若完整保留路径平均敏感度及其导数，则矩阵表达在
数学上就是 Q 的另一种写法，而不是独立的参数权重方案。

### 4.3 机制解释的证据层级

本文的机制解释分四层。第一层是数学恒等式：P 是固定欧氏几何，Q 梯度含当前预测点敏感度，
路径平均割线表达与 Q 精确等价。第二层是局部代理边界：M95 是真值点秩一二次项，存在二维零空间；
24 单元失败消融说明该静态代理在当前合同中不足以替代 Q。第三层是探索性区域关联：真值敏感度
与区域收益相关，但不是必要或充分条件检验。第四层是终态精确观察：Q 的最终参数误差分量发生
更强抵消。前三层说明目标几何具有什么结构及静态解释为何不足，第四层说明拟合后出现了什么；
本文没有记录每一步优化轨迹或对零空间、曲率作正交干预，因此不把“哪一机制贡献多少”或完整 SGD
动力学写成已证因果。
...
P 在归一化参数坐标中使用固定欧氏几何，而 Q 通过当前预测点的误差与敏感度形成动态目标几何；
真值点静态秩一矩阵 M95 在 24 单元探索性消融中未能替代 Q，保存结果则显示参数误差分量通过
更强抵消形成更小的净目标误差。探索性工程审计进一步表明，均方误差收益主要来自压低寿命高估侧和极端
...
*科学主线的机制闭环修订已通过独立方法与图文复核，当前等待用户审阅；此状态不等于期刊同行评议或录用。*
