...
> 状态：`INDEPENDENT METHODS AND CONSISTENCY REVIEW PASSED — WORKING MANUSCRIPT, NOT SUBMITTED`
...
5. 将多 seed、配对 bootstrap 和证据完整性明确定位为可信性验证，而非新的科学问题。
...
更重要的是，\(H_Q=2ss^\mathsf T\) 包含参数方向之间的交叉项：沿目标等值面的参数组合变化在一阶
近似下可以互相补偿；\(H_P=2I\) 则分别惩罚每个坐标偏差。冻结 40 点上 \(\|s\|\) 为
0.766–4.393，方向最大相差 20.3°。因此，不仅一组固定对角权重不能逐点复制 Q，即使一组固定
完整矩阵也不能同时复制全域内不断改变的幅度和方向。Q 不是额外学习三个全局权重，而是由目标公式
和链式法则在当前预测位置生成局部几何。传播到网络更新时还要乘解码器与共享网络 Jacobian，并受
优化器影响。
...
形成更小的净目标误差。探索性工程审计进一步表明，均方误差收益主要来自压低寿命高估侧和极端
...
`artifacts/pq_paper_core/analysis/`，工程误差审计见 `artifacts/pq_engineering_audit/`。
