...
作为对比，Default δ = 0.1 对应 J₁ = 0.6332。两者差异为 0.048%（J₁ 差值 < 0.001），在工程上可忽略（**表 3**）。从全局视角看，δ = 0.1 确实近似最优——但这并不意味着"没有改进空间"，原因见 §3。

**Table 3.** L1 最优 δ\* vs Default — 全局与分 β
...
结论：**全局常数 δ 和按 n 查表均无法系统性地超越经验值 0.1。** 原文的 0.1 在可部署层级（L1/L2）上确实难以改进——这验证了谢里阳等(182-046)的经验选择。但 L1/L2 收益极小，并不等于没有改进空间；改进空间来自更高信息层级，尤其 β 异质性——第 5 章在 oracle 条件下回答这一问题。
