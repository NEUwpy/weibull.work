...
作为对比，Default δ = 0.1 对应 J₁ = 0.6332。两者差异为 0.048%，在统计上不显著（**表 2**）。从全局视角看，δ = 0.1 确实近似最优——但这并不意味着"没有改进空间"，原因见 §3。
...
δ-risk 曲线在 δ ∈ [0.06, 0.12] 区间极为平坦：δ 从 0.06 到 0.12，J₁ 波动不超过 0.6%。这一平坦区间的存在解释了为什么原文"随手取 δ = 0.1"仍然有效——在全局聚合层面，0.1 落在了一个对精度不敏感的平台上。

> **Table 2：全局 δ-risk 关键点**
>
> | δ | J₁ (全局) | 说明 |
> |---|---|---|
> | 0.00 | 0.9796 | 无偏移，极差 |
...
**结论：按 n 分层（L2）相比全局常数（L1）几乎没有收益。** 样本量 n 不是 δ 最优值的主要决定因素。
...
| β = 4.0 | **L1 稳定优于 Default** | +2.1% ~ +3.5% |
| β = 5.0 | **L1 稳定优于 Default** | +2.9% ~ +4.3% |
...
γ/η 的效应相对次要：相同 β 下，γ/η = 0.1（小位置参数）时 L1 改善最大或恶化最小，γ/η ≥ 0.5 时 L1 的劣势更明显。这一现象与 MDM 在大位置参数场合的搜索敏感性有关，但不是本章的主要发现。

## §4 本章小结

本章回答论文主问题 Q1：**δ = 0.1 是否为最优全局常数？**

- **全局层面**：L1 最优 δ\* = 0.08 与 Default 0.1 差异为 0.048%（J₁: 0.6329 vs 0.6332），统计上不显著。δ ∈ [0.06, 0.12] 为平坦区间。
- **按 n 分层**：L2 相比 L1 改善仅 0.059%，样本量不是 δ 最优值的主要决定因素。
- **分参数视角**：全局常数的"近似最优"是不同 β 条件下最优 δ 方向相反、聚合后互相抵消的结果。β = 4.0–5.0 时 L1 稳定优于 Default 2–4%，β = 1.5 时 L1 反而恶化 1–8%。

结论：**全局常数 δ 和按 n 查表均无法系统性地超越经验值 0.1。** 原文的 0.1 在可部署层级（L1/L2）上确实难以改进——这验证了谢里阳等(182-046)的经验选择。但精度提升的空间存在于更高信息层级：如果能根据条件（尤其 β）自适应选择 δ，精度上限在哪里？第 5 章在 oracle 条件下回答这一问题。
