...
4 张图：
  Fig A (fig_offset_mechanism): δ 机制/波动诊断三子图（Ch2 §4，Figure 1）
    ...
    - Panel B：δ=0 与 δ=0.1 的 γ̂ ECDF（MC R=1000，数据源 mc_scan_raw.csv）
    - Panel C：同一 repeat_id 下两种判据的绝对归一化误差配对散点
...
def prepare_paired_offset_diagnostics(rows, gamma_true, eta_true):
    """按 repeat_id 严格配对 δ=0 与 δ=0.1 的位置参数估计。

    Figure 1 Panel C 必须比较同一 Monte Carlo 样本在两种判据下的误差。
    缺失或重复配对会破坏逐样本改善/变差判断，因此直接报错，不做
    valid-only 或隐式删行。
    """
    required = {"repeat_id", "delta", "gamma_hat"}
    missing = required.difference(rows.columns)
    if missing:
        raise ValueError(f"missing required columns: {sorted(missing)}")

    selected = rows[rows["delta"].isin([0.0, 0.1])].copy()
    counts = selected.groupby(["repeat_id", "delta"]).size()
    if (counts > 1).any():
        raise ValueError("duplicate repeat_id/delta observations prevent strict pairing")

    pivot = selected.pivot(index="repeat_id", columns="delta", values="gamma_hat")
    if 0.0 not in pivot.columns or 0.1 not in pivot.columns or pivot[[0.0, 0.1]].isna().any().any():
        raise ValueError("complete paired observations are required for delta=0 and delta=0.1")

    pivot = pivot.sort_index()
    err_zero = (pivot[0.0].to_numpy(dtype=float) - gamma_true) / eta_true
    err_offset = (pivot[0.1].to_numpy(dtype=float) - gamma_true) / eta_true
    abs_zero = np.abs(err_zero)
    abs_offset = np.abs(err_offset)
    effect = np.where(
        np.isclose(abs_offset, abs_zero, rtol=0.0, atol=1e-12),
        "tied",
        np.where(abs_offset < abs_zero, "improved", "worsened"),
    )

    return pd.DataFrame({
        "repeat_id": pivot.index.to_numpy(),
        "gamma_hat_zero": pivot[0.0].to_numpy(dtype=float),
        "gamma_hat_offset": pivot[0.1].to_numpy(dtype=float),
        "error_zero": err_zero,
        "error_offset": err_offset,
        "abs_error_zero": abs_zero,
        "abs_error_offset": abs_offset,
        "effect": effect,
    })
...
def plot_fig_offset_mechanism():
    ...
    三 panel 共享代表配置 β=2.0, η=1000, γ=1000, n=7（贴近 182-046 语境）。
      ...
      - Panel C：同一 repeat_id 下绝对归一化误差的配对散点；对角线下方
        为 δ=0.1 改善，上方为变差。
    """
    ...
    for label, r, _, _ in plot_items:
        print(f"    {label} (rid={r['rid']}): γ_zero={r['gamma_zero_curve']:.1f} "
              f"(err {r['err_zero_curve']:+.1f}%) -> "
              f"γ_offset={r['gamma_offset_solver']:.1f} "
              f"(err {r['err_offset']:+.1f}%)")

    fig = plt.figure(figsize=(7.2, 2.65))
    gs = gridspec.GridSpec(1, 3, width_ratios=[1.25, 0.9, 0.9],
                            wspace=0.36, figure=fig)
    ...
    ax_a.axvline(gamma_true, color="#999999", linewidth=0.6,
               linestyle=":", zorder=1)
    ax_a.text(gamma_true, 0.585, r"True $\gamma$", fontsize=5,
    ...
    ax_a.legend(loc="upper left", fontsize=4.8, framealpha=0.85,
              edgecolor="#cccccc")


    # ── Panel B：δ=0 vs δ=0.1 的 γ̂ ECDF ──
    ...
    paired = prepare_paired_offset_diagnostics(sub, gamma_true=1.0, eta_true=1.0)
    # eta=1 数据乘 1000 对齐 Panel A 的 γ=1000 语境
    gh0 = paired["gamma_hat_zero"].to_numpy() * 1000.0
    gh1 = paired["gamma_hat_offset"].to_numpy() * 1000.0

    for values, color, label in [
        (gh0, "#0072B2", r"$\delta=0$ (zero-grad)"),
        (gh1, "#D55E00", r"$\delta=0.1$ (offset)"),
    ]:
        x = np.sort(values)
        y = np.arange(1, len(x) + 1) / len(x)
        ax_b.step(x, y, where="post", color=color, linewidth=1.1, label=label)
    ax_b.axvline(1000.0, color="#999999", linewidth=0.8,
                 linestyle=":", zorder=2)
    boundary_mass = np.mean(np.isclose(gh0, 0.0))
    ax_b.text(0.04, 0.96,
              f"$\\delta=0$ boundary mass: {boundary_mass:.1%}",
              transform=ax_b.transAxes, fontsize=5, color="#0072B2",
              ha="left", va="top")
    ax_b.set_xlabel(r"Location estimate $\hat{\gamma}$")
    ax_b.set_ylabel("Empirical cumulative probability")
    ax_b.set_ylim(0.0, 1.02)
    ax_b.set_title(r"(b) Population shift in $\hat{\gamma}$",
                   fontweight="bold", loc="left", fontsize=7)
    ax_b.legend(loc="lower right", fontsize=5.2, framealpha=0.85,
                edgecolor="#cccccc")

    # ── Panel C：同一 repeat_id 的绝对归一化误差配对比较 ──
    effect_colors = {
        "improved": "#0072B2",
        "worsened": "#D55E00",
        "tied": "#777777",
    }
    for effect in ["improved", "worsened", "tied"]:
        mask = paired["effect"] == effect
        if not mask.any():
            continue
        ax_c.scatter(
            paired.loc[mask, "abs_error_zero"],
            paired.loc[mask, "abs_error_offset"],
            s=8, alpha=0.38, color=effect_colors[effect], edgecolors="none",
            rasterized=True,
        )
    max_error = float(max(paired["abs_error_zero"].max(),
                          paired["abs_error_offset"].max()))
    limit = max(0.25, np.ceil(max_error * 10) / 10)
    ax_c.plot([0, limit], [0, limit], color="#333333", linewidth=0.8,
              linestyle="--", zorder=1)
    ax_c.set_xlim(0.0, limit)
    ax_c.set_ylim(0.0, limit)
    ax_c.set_aspect("equal", adjustable="box")
    ax_c.set_xlabel(r"$|e_\gamma|$ with $\delta=0$")
    ax_c.set_ylabel(r"$|e_\gamma|$ with $\delta=0.1$")
    improved_rate = np.mean(paired["effect"] == "improved")
    worsened_rate = np.mean(paired["effect"] == "worsened")
    ax_c.text(0.04, 0.96, f"Improved: {improved_rate:.1%}",
              transform=ax_c.transAxes, fontsize=5, color="#0072B2",
              ha="left", va="top")
    ax_c.text(0.04, 0.90, f"Worsened: {worsened_rate:.1%}",
              transform=ax_c.transAxes, fontsize=5, color="#D55E00",
              ha="left", va="top")
    ax_c.set_title("(c) Paired sample-level effect",
                   fontweight="bold", loc="left", fontsize=7)

    # Panel A 标题
    ax_a.set_title(
        r"(a) $\delta$ shifts the gradient criterion"
        "\n(per-sample effect varies)",
        fontweight="bold", loc="left", fontsize=7,
    )

    # ── Panel B/C 诊断数字 ──
    print(f"  Panel B/C (mc_scan_raw.csv严格配对 n={len(paired)}):")
    print(f"    delta=0:   gamma_hat median={np.median(gh0):.1f}, "
          f"|err| median={np.median(paired['abs_error_zero']):.3f}")
    print(f"    delta=0.1: gamma_hat median={np.median(gh1):.1f}, "
          f"|err| median={np.median(paired['abs_error_offset']):.3f}")
    print(f"    paired effect: improved={improved_rate:.1%}, "
          f"worsened={worsened_rate:.1%}, "
          f"tied={np.mean(paired['effect'] == 'tied'):.1%}")

    out_base = os.path.join(FIG_DIR, "fig_offset_mechanism")
    fig.subplots_adjust(left=0.07, right=0.99, bottom=0.22, top=0.84, wspace=0.36)
