...
**Table 4.** L1–L6 全缓存经验风险阶梯（含 MLE 锚点）
...
**Figure 7.** L6 逐样本最优 δ\* 分布。(a) 全样本直方图：47.46% 的样本最优 δ\* = 0，6.57% 的样本最优 δ\* = 0.50，两个精确端点的含义不同。(b) 分 β 的 boxplot：β 越大，δ\* 分布越集中于低值。β = 1.5 的中位数为 0.04 但 IQR 延伸至 0.46，β = 5.0 的中位数为 0 且 IQR 仅到 0.04。
...
- **L6 是扫描网格内的 hindsight benchmark 但不可部署**：47.46% 的样本 δ\* = 0 反映了无偏移候选的重要性及逐样本偶然性；6.57% 的样本 δ\* = 0.50 则是人为上界截断警示。它可以作为 E3 的 sample-level target test，但结果必须回到 selected-loss 和边界诊断解释，不能被写成可直接部署的目标。
