...
> 中文工作稿 v1.1，与《Study01论文初稿-v1.1.md》配套。由 v1.0 原样复制建立，作为后续审查与精修的活动版本。正文给出完整论证，本附录补充实验设计、训练细节、完整分层结果和复现信息。投稿时应按目标期刊格式将“附录”改为“补充材料”或相应栏目。
...
联合损失 $\ell_i$ 和汇总指标 $J_1$ 沿用正文第 2.2 节的定义。汇总时每个样本权重相同；160 个参数组合的重复次数相同，因此也等价于组合等权。未收敛结果不作静默删除。训练标签中若某一候选偏移量估计失败，以训练折全部有效损失的第 99 百分位数填充，该惩罚值只由训练折计算。正式 Dimensional-RAW-MLP、Default 和 L6 均无失败。WMLE 有 1 个样本未收敛，按预先规定的失败规则计入全部样本结果。
...
| Dimensional-RAW-MLP | 每个 $n$ 的 $\gamma/\eta$ 水平五折留出 | 训练输入标准化、目标标准化和 MLP | 评价未见 $\gamma/\eta$ 水平；每折覆盖全部 8 个 $\beta$ | 样本自适应选择能否用于未参与训练的位置尺度比水平 |

神经网络评价共包含 $4\times5\times3=60$ 个折外模型。Dimensional-RAW、Default 和 L6 使用同一测试样本。两类协议不用于彼此之间的无条件排名。
...
| 内容 | 当前相对路径 |
|---|---|
| 正式方法实现 | `Study/01-study-MDM最小偏移量优化研究/code/run_E6b_dimensional_raw_specialist.py` |
| 参数设计与配置 | `Study/01-study-MDM最小偏移量优化研究/code/dim_raw_config.py` |
| 正式训练与评价清单 | `Study/01-study-MDM最小偏移量优化研究/artifacts/formal/E6_dimensional_raw/specialist/manifest.json` |
| 逐折模型和折外预测 | `Study/01-study-MDM最小偏移量优化研究/artifacts/formal/E6_dimensional_raw/specialist/models/`、`predictions/` |
| 利用初估参数选择偏移量实验 | `Study/01-study-MDM最小偏移量优化研究/artifacts/formal/pg_selector/` |
| 未见 $\beta$ 验证 | `Study/01-study-MDM最小偏移量优化研究/artifacts/formal/E6_dimensional_raw/unseen_beta/` |
| 传统方法参照 | `Study/01-study-MDM最小偏移量优化研究/artifacts/formal/E6_dimensional_raw/traditional_ref/` |
| 可靠度寿命 | `Study/01-study-MDM最小偏移量优化研究/artifacts/formal/E6_dimensional_raw/quantiles/` |
...
**表 B1  Dimensional-RAW-MLP 的训练设置。**
...
| 随机种子 | 42、2026、3407 |
| 输入标准化 | 训练折内逐位置 StandardScaler |
| 目标标准化 | 训练折内 26 维 StandardScaler |
| 训练损失 | 标准化后 26 维预测曲线与实际曲线的平方误差 |

### B.2 随机初始化稳定性

表 B2 给出三个随机种子的汇总与分样本量结果。随机种子影响权重初值、训练数据内部验证划分和优化过程，三次训练均无估计失败。

**表 B2  三个随机种子的汇总与分样本量结果。**

| 随机种子 | 汇总 $J_1$ | $n=7$ | $n=10$ | $n=15$ | $n=20$ | 失败率 |
|---:|---:|---:|---:|---:|---:|---:|
| 42 | 0.5545 | 0.6868 | 0.5790 | 0.4857 | 0.4324 | 0% |
| 2026 | 0.5523 | 0.6905 | 0.5739 | 0.4817 | 0.4266 | 0% |
| 3407 | 0.5560 | 0.6932 | 0.5753 | 0.4892 | 0.4312 | 0% |

### B.3 有量纲输入与均值归一化候选

在同一 160 组合设计、相同测试样本和相同三个随机种子下，保留绝对水平的 Dimensional-RAW-MLP 汇总 $J_1$ 为 0.5543，逐样本均值归一化候选为 0.5850。两者都优于固定 $\delta=0.1$ 的 0.6304，但归一化后收益缩小。该结果说明，在当前 $\eta=1000$ 和固定单位的设计中，样本绝对水平包含有助于偏移量选择的信息。

这一比较不支持“有量纲输入普遍优于归一化输入”的结论。保留绝对水平的代价是模型不具备跨单位和跨尺度不变性。当前模型只能用于与训练条件单位一致、尺度相近的数据；若应用尺度发生明显变化，需要重新训练或另行设计尺度处理方式。

### B.4 三参数联合损失的误差分解

正文使用的 $J_1$ 由 $\beta$、$\eta$ 和 $\gamma$ 三个标准化平方误差共同构成。为判断总体改善是否只来自某一个参数，表 B3 分别给出三个误差分量。自适应方法先在每次预设重复训练内计算三参数误差指标，再对重复训练的指标取平均；Default 使用相同评价样本。该分解只解释联合指标的组成，不替代正文中的主评价。
...
三个参数的标准化 RMSE、绝对误差中位数和第 95 百分位数均有所下降，说明联合 $J_1$ 的改善不是由单一参数独自形成。其中 $\eta$ 的均方误差贡献降幅最大，$\beta$ 的降幅相对较小；三者仍均保留非零的尾部误差，因此这一结果不能解释为所有样本上的参数估计都同步改善。
...
补充实验逐一留出 8 个 $\beta$ 水平，并用其余 7 个水平训练各样本量网络。表 D1 和图 D1 给出各留出水平的完整结果。

**表 D1  逐一留出 $\beta$ 水平时的汇总 $J_1$。**

| 留出 $\beta$ | Dimensional-RAW | Default | L6 |
|---:|---:|---:|---:|
| 1.5 | 0.4873 | 0.5477 | 0.3975 |
| 2.0 | 0.4629 | 0.5205 | 0.4033 |
| 2.5 | 0.4830 | 0.5319 | 0.4366 |
| 3.0 | 0.5105 | 0.5724 | 0.4709 |
| 3.5 | 0.5325 | 0.6136 | 0.4963 |
| 4.0 | 0.5740 | 0.6812 | 0.5373 |
| 4.5 | 0.6032 | 0.7305 | 0.5595 |
| 5.0 | 0.6525 | 0.7898 | 0.5982 |

![未见形状参数验证](figures/supplementary/supp_fig_unseen_beta.png)

**图 D1  未见形状参数水平验证。** 逐一留出八个 $\beta$ 水平时，Dimensional-RAW、Default 和 L6 的 $J_1$。蓝色范围表示 Dimensional-RAW 三个随机种子的结果。
...
**图 E1  传统方法的分样本量参照。** Dimensional-RAW、Default、L6、WMLE 和 LSE 在 $n=7,10,15,20$ 下的 $J_1$。WMLE 的 1 次未收敛保留在全部样本结果中。
...
|---|---|---:|---:|---:|---:|---:|
| Dimensional-RAW | $x_{0.90}$ | 0.0221 | 0.1615 | 0.1096 | 0.3212 | 0.000% |
| Dimensional-RAW | $x_{0.95}$ | 0.0501 | 0.2131 | 0.1402 | 0.4163 | 0.000% |
| Dimensional-RAW | $x_{0.99}$ | 0.1271 | 0.3647 | 0.2286 | 0.7057 | 0.000% |
