...
> 中文工作稿 v1.3，与当前《Study01论文初稿-v1.4.md》配套。正文固定随机种子 42 报告主结果，本附录补充初始化敏感性、实验设计、训练细节、完整分层结果、条件风险机制诊断和复现信息。投稿时应按目标期刊格式将“附录”改为“补充材料”或相应栏目。
...
正文第 2.6 节已经定义网络的输入、输出和偏移量选择过程。表 B1 补充实现所需的训练参数。输入和 26 维目标均只用训练折拟合标准化参数，测试折只执行变换。
...
均值归一化后的第 $i$ 个输入不是彼此独立的手工特征，而是第 $i$ 个次序统计量相对于样本均值的位置。整个向量共同保留样本的相对位置、间距和离散形态。逐个置换某一位置会破坏排序样本的联合结构，所得变化不能直接解释为该位置对网络决策的独立贡献；因此，本文不以单个输入坐标的权重或置换重要性解释 MDM 机制。

现有证据支持的是表示与决策层面的解释。均值、均方根和标准差归一化比较说明不同尺度不变表示的结果接近；有量纲输入对照说明固定尺度下的绝对水平仍含预测信息；正文第 3.4 节的独立诊断则表明，样本实现会改变 MDM 的经验梯度曲线、搜索交点和低风险偏移量。结合 $Z$-only 经验参照，可以确认当前可观测样本中存在尚可进一步利用的信息，但不能据此给出各排序位置的因果贡献，也不能把网络参数本身解释为 MDM 的理论机制。
...
正文第 2.5 节给出了利用初估参数选择偏移量的基本步骤。完整实验由两个初始估计量、三个选择族和两种查询方式组成：初始估计量为 MDM-0.1 和 WMLE；PG-beta、PG-beta-n、PG-full 分别使用 $\hat\beta$、$(\hat\beta,n)$ 和 $(\hat\beta,\hat\gamma/\hat\eta,n)$；查询采用最近网格点或连续插值。单步规则只更新一次，迭代规则在偏移量不再变化、出现循环或达到 10 次更新时终止。条件均值损失曲线只由训练折构造。
...
可靠度寿命沿用正文第 2.9 节的定义，即 $x_R$ 满足 $P(T>x_R)=R$。表 E2 报告正文未逐项列出的相对偏差、相对 RMSE、相对 MAE 和绝对相对误差第 95 百分位数。
