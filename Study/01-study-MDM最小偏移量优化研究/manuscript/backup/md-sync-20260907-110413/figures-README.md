...
**Word 图件（2026-09-07）**：[word/README.md](word/README.md)。四份手稿中的 Study01 正文与附录已使用该目录的 17 张重绘图；生成代码、PNG、可编辑 SVG 和装配清单一并保留。下文原目录说明针对 Markdown 使用的历史资产。
...
原目录图表曾通过技术规范检查，之后作者要求改善 Word 中的字号和清晰度；该项修订现已落入 `word/`。Markdown 的原图链接保持原状。
