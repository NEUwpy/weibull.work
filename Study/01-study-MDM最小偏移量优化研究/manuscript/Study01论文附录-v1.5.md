...
> 中文工作稿 v1.5，与当前《Study01论文初稿-v1.6.md》配套。正文固定随机种子 42 报告主结果，本附录补充初始化敏感性、实验设计、训练细节、完整分层结果、条件风险机制诊断和复现信息。投稿时应按目标期刊格式将“附录”改为“补充材料”或相应栏目。
...
正文固定 seed 42 报告单模型主结果。表 B2 另列两个预设随机种子，展示权重初值、训练数据内部验证划分和优化过程改变后的结果。三条预测曲线分别评价，三次训练均无估计失败。
...
现有结果从表示和决策两个层面解释样本自适应选择。均值、均方根和标准差归一化得到相近结果；有量纲输入表明固定尺度下的绝对水平仍含预测信息；正文第 3.3 节进一步显示，样本实现会改变 MDM 的经验梯度曲线、搜索交点和低风险偏移量。结合 $Z$-only 经验参照，这些结果说明当前可观测样本中仍有可利用信息。排序位置的独立贡献和网络参数的统计含义留待专门研究。

### B.5 逐参数误差的补充分布

正文表 4 已按 $\beta$、$\eta$ 和 $\gamma$ 分别报告标准化 Bias、SD 和 RMSE。表 B3 在相同评价样本上进一步给出绝对误差中位数、第 95 百分位数和各参数均方误差贡献的降幅，用于观察典型误差与上尾误差，不在正文重复展开。
...
在固定当前 160 个设计单元及其等权方式下，配对重复块 bootstrap 给出的汇总改善为 7.27%，95% 置信区间为 6.94%—7.64%。按样本量分层后，四组点估计和区间如表 B4 所示。这些区间以固定的 seed 42 选择器和当前设计为条件，反映有限 Monte Carlo 重复带来的不确定性。表示选择、模型筛选和训练初始化的敏感性通过其他补充实验分别考察。
...
沿用正文第 2.1 节的记号，令 $h=\hat\gamma_\delta-\gamma_\delta$。在两交点均为内点、邻域内可微且余项可忽略时，将经验判据在 $\gamma_\delta$ 处展开，有

$$
0\approx\varepsilon_X(\gamma_\delta)
 +\left[G'_{\theta_0}(\gamma_\delta)+\varepsilon'_X(\gamma_\delta)\right]h.
$$

当扰动的局部斜率相对平均曲线斜率较小，即 $|\varepsilon'_X(\gamma_\delta)|\ll|G'_{\theta_0}(\gamma_\delta)|$ 时，得到正文的一阶式。它要求局部扰动足够小，而不要求扰动在整个搜索区间为常数。

若该近似还能在当前参数条件下取期望和方差，并且相应二阶矩存在，由 $E\{\varepsilon_X(\gamma_\delta)\mid\theta_0\}=0$ 得到领先阶关系

$$
\operatorname{Bias}(\hat\gamma_\delta\mid\theta_0)
 \approx\gamma_\delta-\gamma_0,\qquad
\operatorname{Var}(\hat\gamma_\delta\mid\theta_0)
 \approx\frac{\operatorname{Var}\{\varepsilon_X(\gamma_\delta)\mid\theta_0\}}
 {[G'_{\theta_0}(\gamma_\delta)]^2}.
$$

偏移量改变时，平均交点相对真值的距离、交点处的扰动方差和局部斜率都会变化。这给出了比较偏差与波动来源的局部解释；它们随 $\delta$ 的具体变化方向需要分别核验。对于边界解、局部导数接近零或扰动较大的样本，采用正文的重复抽样结果和实际搜索轨迹评价。
...
该验证覆盖当前八点离散 $\beta$ 网格。参数组合中的局部退化和已训练样本量范围见正文第 3.4 节和第 4.4 节。
...
可靠度寿命沿用正文第 2.8 节的定义，即 $x_R$ 满足 $P(T>x_R)=R$。表 E2 报告正文未逐项列出的相对偏差、相对 RMSE、相对 MAE 和绝对相对误差第 95 百分位数。
