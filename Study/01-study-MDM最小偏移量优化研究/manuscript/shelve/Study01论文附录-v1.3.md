...
现有证据支持的是表示与决策层面的解释。均值、均方根和标准差归一化比较说明不同尺度不变表示的结果接近；有量纲输入对照说明固定尺度下的绝对水平仍含预测信息；正文第 3.3 节的独立诊断则表明，样本实现会改变 MDM 的经验梯度曲线、搜索交点和低风险偏移量。结合 $Z$-only 经验参照，可以确认当前可观测样本中存在尚可进一步利用的信息，但不能据此给出各排序位置的因果贡献，也不能把网络参数本身解释为 MDM 的理论机制。
...
作为补充探索，本附录检验先估计总体参数、再查询相应条件规则的选择方式。完整实验由两个初始估计量、三个选择族和两种查询方式组成：初始估计量为 MDM-0.1 和 WMLE；PG-beta、PG-beta-n、PG-full 分别使用 $\hat\beta$、$(\hat\beta,n)$ 和 $(\hat\beta,\hat\gamma/\hat\eta,n)$；查询采用最近网格点或连续插值。单步规则只更新一次，迭代规则在偏移量不再变化、出现循环或达到 10 次更新时终止。条件均值损失曲线只由训练折构造。该探索不参与正文主方法的论证和比较。
