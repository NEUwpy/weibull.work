...
def draw_figure(results: dict[str, dict[str, float]]) -> plt.Figure:
    ...
    fig = plt.figure(figsize=(180 / 25.4, 112 / 25.4), constrained_layout=False)
    grid = fig.add_gridspec(
        1, 2, width_ratios=[1.08, 0.92],
        left=0.19, right=0.98, bottom=0.21, top=0.89, wspace=0.32,
    )
    ...
    for y, layer in zip(y_positions, LAYERS):
        ...
        if layer == "Default":
            ...
            label = f"{value:.4f}"
        ax_a.text(0.636, y - 0.20, label, ha="right", va="center", color=color, fontsize=8.0)
    ...
    ax_a.set_title("不同信息条件", loc="left", fontsize=10, pad=9)
    ...
    ax_b.set_title("相对固定规则降幅（%）", loc="left", fontsize=10, pad=9)
    ...
    ax_a.text(-.39, 1.045, "a", transform=ax_a.transAxes, fontsize=10, fontweight="bold")
    ax_b.text(-.24, 1.075, "b", transform=ax_b.transAxes, fontsize=10, fontweight="bold")
    return fig
