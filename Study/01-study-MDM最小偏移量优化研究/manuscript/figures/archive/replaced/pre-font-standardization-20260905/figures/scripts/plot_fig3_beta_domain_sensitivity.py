...
def main():
    ...
    fig, axes = plt.subplots(1, 2, figsize=(180 / 25.4, 93 / 25.4),
        gridspec_kw={"width_ratios": [1.28, 1], "wspace": 0.48})
    ...
    choice.set_yticks(y,labels,fontsize=6.6);choice.set_xticks([0,.1,.2,.3])
    choice.set_xlabel(r"偏移量 $\delta$")
    choice.set_title("b  最低点与近优范围",loc="left",fontsize=8)
    choice.grid(axis="x",alpha=.18)
    choice.legend(loc="lower right",fontsize=6.3,frameon=False,bbox_to_anchor=(1.03,0.61))
    for ax in axes:
        ax.spines[['top','right']].set_visible(False)
    export_figure(fig)
