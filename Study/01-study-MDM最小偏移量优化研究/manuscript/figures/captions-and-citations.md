...
**图 3  固定宽度参数域对统一偏移量的影响。** **a，** 在 0.25 间距的 $\beta$ 网格上平移宽度为 1.00 的参数域，每个区间含 5 个连续水平；**b，** 各参数域在 26 个实际候选偏移量上的汇总 $J_1$ 曲线。红线连接每个区间的离散最低点，灰色虚线连接 $\delta=0.10$ 的对应位置；圆点均为实际评价结果，连线只用于引导观察，不表示连续拟合。
