...
**图 5  样本自适应选择在参数空间中的效果分布。** 四个面板分别对应 $n=7,10,15,20$，颜色表示均值归一化 MLP 相对于固定 $\delta=0.1$ 的 pooled $J_1$ 降幅。红框标出发生退化的参数组合；160 个组合中有 35 个退化，最大退化为 17.50%。该图显示汇总改善与局部退化同时存在。
