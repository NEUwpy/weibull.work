...
**图 2  正偏移量与进一步选择的作用。** a，26 个候选偏移量在整个设计域上的汇总 $J_1$；b，低误差区域的局部放大；c，同一 48,000 个样本在无偏移判据（$\delta=0$）、经验偏移量（$\delta=0.10$）和样本自适应选择下的联合误差幅值 $\sqrt{\ell_i}$。细线、粗线、空心圆和菱形分别表示第 5—95 百分位区间、四分位距、中位数和汇总 $J_1$。
