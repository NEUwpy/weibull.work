...
**图 3  不同样本量下的总体与逐样本结果。** **a，** Dimensional-RAW、固定经验偏移量和逐样本 hindsight 参照在四个已训练样本量下的 pooled $J_1$；阴影表示 Default 与 hindsight 之间的逐样本可实现空间，百分数表示样本自适应方法取得该空间的比例。**b，** 逐样本配对损失差 $\Delta\ell_i=\ell_{i,\mathrm{Default}}-\ell_{i,\mathrm{Adaptive}}$ 的分布；细线、中线、粗线和白点依次表示第 1—99、第 5—95 百分位区间、四分位距和中位数，百分数表示误差降低的样本比例。
...
**图 5  样本自适应选择在参数空间中的效果分布。** 四个面板分别对应 $n=7,10,15,20$，颜色表示 Dimensional-RAW 相对于固定 $\delta=0.1$ 的 pooled $J_1$ 降幅。红框标出发生退化的参数组合；160 个组合中共有 3 个退化，且均位于 $n=7$。该图说明总体改善并非掩盖大范围退化，同时保留少数不利区域的边界信息。
...
**图 6  主结果的支撑验证。** a，逐一留出未参与训练的 $\beta$ 水平后，Dimensional-RAW 相对于固定偏移量的 $J_1$ 降幅，阴影为三个随机种子的范围；b，在相同样本量分层下与 WMLE、LSE 的 pooled $J_1$ 外部参照；c，Dimensional-RAW、固定偏移量和 WMLE 对 $x_{0.90}$、$x_{0.95}$、$x_{0.99}$ 的相对 RMSE。三类结果分别回答参数泛化、传统方法参照和参数收益能否传递到工程指标，详细分层结果见补充材料。
...
**图 D1  未见形状参数水平验证。** 逐一留出八个 $\beta$ 水平时，Dimensional-RAW、固定偏移量和事后参照的汇总 $J_1$。蓝色阴影表示 Dimensional-RAW 三个随机种子的范围。

**图 E1  传统方法外部参照。** 在相同 48,000 个样本和评价准则下，比较 Dimensional-RAW、固定偏移量、事后参照、WMLE 和 LSE 在不同样本量下的汇总 $J_1$。

**图 E2  可靠度寿命误差。** 五种方法在 $x_{0.90}$、$x_{0.95}$ 和 $x_{0.99}$ 上的相对 RMSE。Dimensional-RAW 的误差线表示三个随机种子的范围。
