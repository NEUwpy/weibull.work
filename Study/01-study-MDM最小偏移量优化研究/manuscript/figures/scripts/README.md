...
- `qa_submission_figures.py` 核对 60 个导出文件、矢量文本、图像尺寸和核心数值，并验证图 1 的组合内稳定性分布、图 3 的参数域敏感性、图 4 的逐样本配对分布、正文表 4 与附录 B3 的逐参数误差。
...
当前投稿图的唯一绘制入口是本目录的 `make_submission_figures.py`；它只读取正式封存数据并在论文侧生成 7 张正文图和 8 张补充/复核图。投稿图全部使用 Python/matplotlib 生成，导出 PNG、SVG、PDF 和 TIFF。正式封存数据保持只读；不要直接修改正式目录中的 PNG。被替换的旧图保存在 `../archive/replaced/`，不直接删除。
