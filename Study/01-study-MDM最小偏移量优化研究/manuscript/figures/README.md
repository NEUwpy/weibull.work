...
当前活动稿使用 12 张图，均已按 SCI 投稿图标准完成重画：
...
- 图 5 展示同一参数条件内的事后偏移量多样性，并比较不同决策条件的确认集风险；
- 图 6 用一张复合图概括未见参数、传统估计方法和可靠度寿命三类支撑验证；
- 6 张稿件引用的补充图统一了方法颜色、字号、线宽、图例和误差范围定义；seed 稳定性由附录表 B2 报告，另保留一张可复核但不在稿件重复引用的稳定性图；
...
SCI 图像应当“信息丰富而不拥挤”。目前六张正文图依次承担方法、统一规则、核心效果、选择过程、条件风险机制和支撑验证，六张稿件引用的补充图保存可追问的分层细节。后续不再为装饰性丰富增加重复面板。
