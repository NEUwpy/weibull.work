...
> 中文修订稿 v1.10，与[正文 v1.12](Study01论文初稿-v1.12.md)配套。本附录补充实验设计、训练细节、初始化敏感性、完整分层结果及复现信息。

> 研究导航：[正文—实验对应表](../01-证据索引.md) · [实验配置 S01-01—S01-08](../02-实验配置.md)。
...
形状参数域敏感性分析的补充设计如下。为检验统一低风险偏移量是否随参数域改变，另将 $\beta$ 加密为 1.50—5.00、步长 0.25，并构造宽度固定为 1.00、包含 5 个连续水平的滑动区间。区间中心由 2.00 移至 4.50，其余 $\gamma/\eta$、$n$、候选偏移量和参数条件权重均保持不变。既有 8 个 $\beta$ 水平复用主扫描的前 100 次重复，仅对 7 个新增水平按同一抽样和 MDM 入口补算。每个区间分别计算 26 点汇总 $J_1$ 曲线、离散最低点、$\delta=0.10$ 相对最低点的超额 $J_1$ 和 1% 近优区间；最低点的稳定性用 5,000 次重复块 bootstrap 描述。

### A.2 损失、失败结果与汇总方式

联合损失 $\ell_i$ 和汇总指标 $J_1$ 沿用正文第 2.2 节的定义。汇总时每个样本权重相同；160 个参数组合的重复次数相同，因此也等价于组合等权。未收敛结果不作静默删除。训练标签中若某一候选偏移量估计失败，以训练折全部有效损失的第 99 百分位数填充，该惩罚值只由训练折计算。正式均值归一化 MLP、Default 和 L6 均无失败。WMLE 有 1 个样本未收敛，将该次失败的三个估计值记为零后代入联合损失，所得 $\ell_{\mathrm{fail}}=2+(\gamma/\eta)^2$ 计入全部样本结果，同时报告失败率。该数值是失败计分约定，不是有效参数估计；完整案例结果另作敏感性参照。
...
神经网络评价共包含 $4\times5\times3=60$ 个折外模型。均值归一化 MLP、Default 和 L6 使用同一测试样本。信息层级与神经网络结果按照各自的评价协议分别报告。
...
| L2 | 样本量 $n$ | 4 |
| L3 | 形状参数 $\beta$ | 8 |
| L4 | 形状参数 $\beta$ 与样本量 $n$ | 32 |
| L5 | 完整参数组合 | 160 |

注：各规则在每组训练样本的平均损失曲线上选择偏移量。
...
<a id="app-a5"></a>
### A.5 [L6 候选上边界核查](../02-实验配置.md#s01-08)

为检查 $\delta=0.50$ 处仍在下降的损失曲线，对满足 $\ell(0.50)<\ell(0.48)$ 的 2,186 个样本将候选网格延伸至 1.00，其余样本沿用原扫描。延伸后全样本 L6 的 $J_1$ 从 0.492297 降至 0.490848，以原网格上的 $J_1$ 为基准计算相对降幅，约为 0.29%；相应均方损失 $\mathcal R=J_1^2$ 的降幅约为 0.59%。仍有 590 个样本在新上界 1.00 取最小损失。该分析衡量所选右端下降样本对全样本参照的影响；正文保留原定 26 点网格上的 L6。
...
| 批量大小 | 256 |
| 最大迭代次数 | 300 |
| 提前停止 | 外层训练折中 15% 作内部验证；按验证集多输出 $R^2$（均匀平均）监测，容差 $10^{-4}$、连续 20 次迭代无充分改善时停止，并恢复验证得分最高的权重 |
...
在相同 160 组合设计、相同折外样本和相同网络配置与评价协议下，额外比较了三种已有的正尺度不变表示：排序样本除以样本均值、样本均方根和样本标准差。三种表示在候选筛选中的汇总 $J_1$ 分别为 0.5850、0.5854 和 0.5894。均值与均方根归一化的结果接近；本文选用前者，是因为它在该次筛选中的 $J_1$ 略低、定义直接，并已有完整折外模型可供主结果和敏感性核查。表示筛选与主结果使用同一批折外样本；配对 bootstrap 条件于现有预测，未计入表示筛选的不确定性。独立确认的安排见正文第 4.4 节。

作为输入表示的敏感性参照，保留绝对水平的 Dimensional-RAW-MLP 在同一固定 $\eta=1000$ 设计下的 $J_1$ 为 0.5543，低于归一化方法。这表明绝对水平在该固定尺度任务中具有预测信息，同时也显示了去除这一信息带来的精度代价。论文主方法采用具有正尺度不变性的均值归一化表示。

对样本均值非零的 $X$ 和正数 $c$，有 $Z(cX)=cX/(c\bar X)=Z(X)$。固定训练得到的标准化器、网络和候选平局处理规则后，预测损失与所选 $\delta$ 均不变。进一步，由正文的伪尺度参数定义，

$$
\eta_i^{(cX)}(\beta,c\gamma)=c\eta_i^{(X)}(\beta,\gamma),\qquad
S_{cX}(c\gamma)=cS_X(\gamma).
$$

在相同的形状参数搜索区间内，乘以 $c>0$ 不改变标准差最小化的形状参数解集；在唯一解或采用等变选解规则时，$\beta^*_{cX}(c\gamma)=\beta^*_X(\gamma)$。可微处满足

$$
\nabla_{cX}(c\gamma)=\nabla_X(\gamma).
$$

因此，$S$ 与 $\gamma$ 同量纲，梯度 $\nabla$ 及偏移量 $\delta$ 均无量纲。位置参数搜索域 $[0,t_{(1)})$ 随尺度同倍变化，$\gamma=0$ 的边界判据保持不变；对应的精确 MDM 解满足 $\hat\beta(cX)=\hat\beta(X)$、$\hat\eta(cX)=c\hat\eta(X)$ 和 $\hat\gamma(cX)=c\hat\gamma(X)$。真值同时变换为 $(\beta,c\eta,c\gamma)$ 时，正文的标准化联合损失也保持不变。

为核查有限数值精度下的实现，选取覆盖 $n=7,10,15,20$ 和参数边界与中间水平的 4 个 Monte Carlo 样本，分别运行 $X\times10^{-3}$、$X$ 和 $X\times10^3$，共 12 次生产 MDM。所有尺度下的预测损失曲线和所选 $\delta$ 一致，$\hat\beta$ 不变，$\hat\eta$ 和 $\hat\gamma$ 在数值容差内按 $c$ 倍变化，标准化损失保持一致。

### B.4 输入信息与解释范围

均值归一化后的第 $i$ 个输入表示第 $i$ 个次序统计量相对于样本均值的位置，整个向量共同保留样本的相对位置、间距和离散形态。单独置换某一位置会同时破坏排序样本的联合结构，因此输入解释以完整排序向量及其分布形态为对象。

现有结果从表示和决策两个层面解释样本自适应选择。均值、均方根和标准差归一化得到相近结果；有量纲输入表明固定尺度下的绝对水平仍含预测信息；正文第 3.4 节进一步显示，样本实现会改变 MDM 的经验梯度曲线、搜索交点和低风险偏移量。结合 $Z$-only 经验参照，这些结果说明当前可观测样本中仍有可利用信息。排序位置的独立贡献和网络参数的统计含义留待专门研究。

### B.5 逐参数误差的补充分布

正文表 4 的 SD 是混合设计下标准化误差的标准差。令 $C$ 为参数组合、$e$ 为某参数的标准化误差，由全方差公式，

$$
\operatorname{Var}(e)=\operatorname{E}_C\!\left[\operatorname{Var}(e\mid C)\right]
+\operatorname{Var}_C\!\left[\operatorname{E}(e\mid C)\right].
$$

正文表 4 已按 $\beta$、$\eta$ 和 $\gamma$ 分别报告标准化 Bias、SD 和 RMSE。表 B3 在相同评价样本上进一步给出绝对误差中位数、第 95 百分位数和各参数均方误差贡献的降幅，用于观察典型误差与上尾误差。
...
设计单元之间的改善幅度存在明显差异。160 个单元中，125 个的单元 $J_1$ 低于 Default，35 个更高；单元相对改善的中位数为 7.40%，四分位区间为 1.39%—12.65%，范围为 $[-17.50\%,19.98\%]$。将 160 个单元均值配对重采样时，汇总降幅的 95% 设计构成敏感性范围为 6.04%—8.46%，用于衡量汇总结果随设计单元构成变化的程度。

### B.7 共同可行区间内的局部交点差

固定样本量和两组样本 $X^{\mathrm{ref}},X$。记参考梯度曲线为 $g_{\mathrm{ref}}(\gamma)=\nabla_{X^{\mathrm{ref}}}(\gamma)$，曲线差为 $e_X(\gamma)=\nabla_X(\gamma)-g_{\mathrm{ref}}(\gamma)$，并选取两条曲线搜索域交集内的一个邻域。若参考交点 $\hat\gamma_{\delta,\mathrm{ref}}$ 和待比较交点 $\hat\gamma_{\delta,X}$ 均位于该邻域，令二者之差为 $h$，一阶展开得

$$
0\approx e_X(\hat\gamma_{\delta,\mathrm{ref}})
+[g'_{\mathrm{ref}}(\hat\gamma_{\delta,\mathrm{ref}})
+e'_X(\hat\gamma_{\delta,\mathrm{ref}})]h.
$$

当 $|e'_X|\ll|g'_{\mathrm{ref}}|$ 且参考斜率非零时，得到一阶交点差
...
它是两组给定样本曲线之间的局部关系，不要求将随机搜索域外的梯度延拓，也未定义总体平均交点。对边界解、非光滑点、近零斜率或大扰动样本，该近似不适用。本文没有由此推导总体 Bias 或方差；稳定性和估计风险均由实际重复抽样结果评价。

### B.8 评价模型、应用模型与计算成本

主 seed 的位置尺度比留出评价包含 $4\times5=20$ 个模型，每个评价样本只使用未纳入其位置尺度比水平训练的模型。附录的另外两个 seed 用于敏感性核查。本文主结果汇总这 20 个折外模型的预测；全部设计单元上的重训模型在附录 F 中单独评价。

应用过程需要针对对应样本量保存的输入标准化器、目标标准化器和网络。若在全部设计数据上重训每个 $n$ 的最终模型，应固定本附录的表示与训练规则，并另外确认其表现；本文没有将这一步作为已完成的正式主验证。
...
图 B1a 给出一个折外测试样本，其实际损失在 $\delta=0.14$ 处最低，而网络选择了 $\delta=0.22$。预测曲线识别了实际曲线中的低损失区域，全部测试样本的选择结果与 L6 偏移量存在不同程度的偏离（图 B1b）。网络通过提高低损失区域的选择频率降低了汇总风险。
...
<a id="app-c"></a>
## 附录 C  [利用初估参数选择偏移量](../02-实验配置.md#s01-07)

### C.1 初估参数选择规则

作为补充探索，本附录检验先估计总体参数、再查询相应条件规则的选择方式。完整实验由两个初始估计量、三个选择族和两种查询方式组成：初始估计量为 MDM-0.1 和 WMLE；PG-beta、PG-beta-n、PG-full 分别使用 $\hat\beta$、$(\hat\beta,n)$ 和 $(\hat\beta,\hat\gamma/\hat\eta,n)$；查询采用最近网格点或连续插值。单步规则更新一次，迭代规则在偏移量稳定、出现循环或达到 10 次更新时终止。条件均值损失曲线由训练折构造。
...
表 C2 给出最佳单步规则按真 $\beta$ 的完整结果。初步 $\hat\beta$ 的网格单元正确率与连续插值结果共同用于分析初估误差和规则表现。
...
<a id="app-d"></a>
## 附录 D  [未见形状参数水平验证](../02-实验配置.md#s01-06a)
...
该验证覆盖当前八点离散 $\beta$ 网格。参数组合中的局部退化和已训练样本量范围见正文第 3.5 节和第 4.4 节。

<a id="app-e"></a>
## 附录 E  [传统方法参照与可靠度寿命](../02-实验配置.md#s01-06b)
...
可靠度寿命沿用正文第 2.6 节的定义，即 $x_R$ 满足 $P(T>x_R)=R$。表 E2 报告正文未逐项列出的相对偏差、相对 RMSE、相对 MAE 和绝对相对误差第 95 百分位数。
...
令 $a_R=-\ln R$、$w_R=a_R^{1/\beta}$。对 $x_R=\gamma+\eta w_R$ 在真参数处作局部展开，得到

$$
\Delta x_R\approx
-\frac{\eta w_R\ln a_R}{\beta^2}\Delta\beta
+w_R\Delta\eta+\Delta\gamma.
$$

记上式三个系数组成的向量为 $\boldsymbol d_R$，参数估计误差的均值和协方差矩阵为 $\boldsymbol b$ 与 $\boldsymbol\Sigma$，则在一阶近似适用时，

$$
E[(\Delta x_R)^2]\approx
(\boldsymbol d_R^{\mathsf T}\boldsymbol b)^2+
\boldsymbol d_R^{\mathsf T}\boldsymbol\Sigma\boldsymbol d_R.
$$

与参数联合损失相比，寿命误差采用随 $R$ 和真参数变化的权重，并包含交叉协方差项。因此，逐参数均方误差的下降与某个寿命点的改善可以具有不同幅度。此处展开用于解释两类评价的区别；表 E2 的数值均直接由各样本估计出的寿命计算。

<a id="app-f"></a>
## 附录 F  [条件风险与样本实现](../02-实验配置.md#s01-05)
...
本实验构造当前设计上的 $Z$-only 经验风险参照。160 个设计单元中，repeat 0–159 用于候选拟合，160–199 用于选模；选定回归器后使用 0–199 重新拟合，200–299 作为独立确认集。每个 $n$ 独立选择回归器，候选包括 Ridge、KNN、ExtraTrees、当前 MLP 结构和较宽 MLP。$n=7,15,20$ 选中 ExtraTrees，$n=10$ 选中较宽 MLP。同结构 MLP 对照也使用 0–199 重新拟合。训练器接收 $Z$ 和 26 维 Monte Carlo 损失标签；真参数用于构造标签、L5/L6 参照和事后评分。确认集共含 16,000 个唯一样本，各参数单元均有 100 个独立重复。

### F.2 确认集风险

**表 F1  不同决策条件下的确认集风险。**

| 决策条件 | 确认集均方损失 $\mathcal R$ | $J_1=\sqrt{\mathcal R}$ | 信息口径 |
|---|---:|---:|---|
...
| 灵活 $Z$-only 经验参照 | 0.315658 | 0.561835 | 所有设计单元参与拟合；验证选模后在独立确认集评分 |
| L6 | 0.240646 | 0.490557 | 逐样本已实现 26 点损失全部已知 |

灵活 $Z$-only 参照相对论文 MLP 的配对风险差为 $-0.023873$，repeat-block bootstrap 95% 置信区间为 $[-0.025405,-0.022366]$。相对同域当前 MLP 结构的差为 $-0.007010$，95% 置信区间为 $[-0.008727,-0.005408]$。这说明当前 MLP 尚未充分利用当前可观测输入中的信息。论文 MLP 与同域重训结构的差异同时包含参数覆盖、评价协议和重新拟合的变化。

为展示可加的风险差，定义路径中相邻规则 $A\to B$ 的占比为

$$
p_{A\to B}=\frac{\mathcal R_A-\mathcal R_B}
{\mathcal R_{\mathrm{Default}}-\mathcal R_{L6}}\times100\%.
$$
...
$$
\nabla_X(\gamma)=\frac{\partial}{\partial\gamma}
