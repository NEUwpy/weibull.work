...
第 4 章已经说明，L2 相对 L1 的 pooled $J_1$ 降幅为 0.059%，未呈现与按 $\beta$ 分组的 0.77%–8.11% 变化同量级的改善；但 `n` 仍然是部署时已知且审稿人最关心的小样本维度。E3b 因此必须检查每个 `n` 下的方向是否一致。
...
第一，E3b 是 **existing-grid** 结论。训练和测试均来自本文正式离散网格，只是通过 full-combo holdout 避免同一参数组合的 repeat 同时出现在训练和测试中。因此，E3b 支持的是：使用部署时可获得的输入，`Vector-MLP-L6` 在当前正式离散网格的 combo-holdout 评价中取得改进。该结果尚不足以把模型或其选择信号称为“可部署”，也不能单独支撑“连续参数空间上稳定泛化”的部署推荐；这些判断仍需 Ch7 的边界检查和真实数据验证。
...
- near-optimal 诊断显示，`Vector-MLP-L6` 的 near-5% hit rate 相比 L2 由 18.78% 提高到 40.90%，为选点改善提供了逐样本诊断证据；该结果不识别模型学到的内部表示，也不排除未经测试的固定规则。
