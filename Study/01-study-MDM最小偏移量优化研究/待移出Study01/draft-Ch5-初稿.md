...
第 4 章表明，在当前正式网格的等权 pooled 口径下，L1 相对 Default、L2 相对 L1 的 $J_1$ 降幅分别为 0.048% 和 0.059%，均未呈现与按 $\beta$ 分组的 0.77%–8.11% 变化同量级的改善，因此经验值 δ = 0.1 仍是强基线。一个自然的问题是：如果**知道更多**——不仅知道 n，还知道 β、γ/η，甚至逐样本——精度能提升多少？本章在 oracle 条件下（真参数已知）回答这一问题，量化每一级信息的边际价值，并给出扫描网格内的逐样本 hindsight 参照。
...
cross-fit 后，L2→L3 改善 7.53%，L3→L4 改善 0.42%，L4→L5 改善 1.83%。L1 的全局选点和 L3 的 5 个 $\beta$ 选点在五折中完全稳定；L2 的 3 个分组中有 2 个完全稳定，L4 为 10/15，L5 为 28/45。相对同批选点/评价，L2、L4 和 L5 的留出 $J_1$ 分别上调 0.030%、0.085% 和 0.132%，最大上调为 0.132%；留出 $J_1$ 依次为 L2=0.632732、L3=0.585068、L4=0.582585、L5=0.571924，L2→L5 的层级顺序保持不变。
...
## §2 L3：引入 $\beta$ 组级信息带来 7.51% 的逐级降幅
...
**$\beta$ 分组对应怎样的选点与 profile 几何变化？** 当前结果直接支持的经验关系是：$\beta$ 越小，最优 $\delta$ 越大；$\beta$ 越大，最优 $\delta$ 越接近低值。为检查这一关系是否伴随 profile 曲线几何变化，我们在固定 $\eta=1$、$\gamma/\eta=0.5$ 下重建 5 个 $\beta$、3 个 $n$、每格 20 个 repeat，共 300 个正式 seed 样本，并提取真实 $\gamma$ 邻域内的局部梯度斜率作为曲率代理。$\beta$ 与该代理的 Spearman $\rho$ 在 $n=7/10/20$ 下分别为 −0.463、−0.495 和 −0.529，三个样本量下方向一致；这说明有限样本 profile 曲线几何随 $\beta$ 系统变化，与上述机制解释一致。不过，真实 $\gamma$ 处梯度的方向并未跨 $n$ 保持一致，且该审计没有识别“尾部形态 → profile 几何 → 最优 $\delta$”的因果链。因此，该审计只支持 $\beta$ 分组与选点、profile 几何之间的经验关联，不将尾部形态解释写成已证明机制。

在可比较的组级信息层级 L2→L5 中，最大的逐级相对 $J_1$ 降幅是 L2→L3 的 7.51%；L3 只使用 5 个 $\delta^*$ 值，每个 $\beta$ 对应一个。L5→L6 的 13.42% 来自从组级选择切换为逐样本事后选择，属于 hindsight gap，不纳入组级信息增益排序。这为第 6 章提供了任务方向：样本自适应方法应优先学习与 $\beta$ 相关的可观测形态特征，同时必须检查这种学习在不同 $n$ 下是否稳定。
...
1. **下端点 δ\* = 0 的堆积**。47.46% 的样本最优 δ\* 恰好为零。这不是说"这些样本不需要偏移"，而是 δ grid 上具有方法含义的“无偏移”候选 δ = 0 恰好对这些 MC 样本给出了最低的 $\ell_i(\delta)$。该比例描述扫描网格内的下端点质量，本身不区分结构性样本差异与 Monte Carlo 随机性。
...
4. **L6 可以作为压力测试参照，但不能写成部署目标**。L5 的 J₁ = 0.5712 是"组级真参数信息能达成的参照"，而样本自适应方法的任务是用可观测样本特征尽量逼近或解释 oracle/hindsight 阶梯中的可学习部分。当前证据不分解 13.42% hindsight gap 的结构性与随机性来源；47.46% 的下端点质量、6.57% 的上端点质量及分 $\beta$ 后仍存在的组内分散，只说明该差距不能直接解释为可部署信息收益。L6 因此可作为 sample-level hindsight target test 或离线压力测试参照；若 E3 使用 L6 监督粒度，必须用真实 selected-loss J₁、near-optimal/regret 和边界诊断解释结果，不能把 L6 写成实际部署目标。
...
- **Figure 7 解释 L6 为何是 hindsight benchmark 而不是部署查表**：47.46% 的样本 $\delta^*=0$ 描述无偏移候选的下端点质量；6.57% 的样本 $\delta^*=0.50$ 是人为上界截断警示；分 $\beta$ 后仍可见组内逐样本分散。现有证据不分解这些现象的结构性与随机性来源。L6 可以作为 E3 的 sample-level target test，但结果必须回到 selected-loss 和边界诊断解释，不能被写成可直接部署的目标。

结论：**在 L2→L5 的组级信息层级中，L2→L3 的 7.51% 是最大的逐级相对 $J_1$ 降幅；$n$ 仍是部署和审稿解释中不可省略的维度。** L3→L4 和 L4→L5 的降幅分别为 0.51% 和 1.88%；L5→L6 的 13.42% 单独定义为逐样本 hindsight gap，不进入组级信息增益排序。这意味着第 6 章的样本自适应方法应优先学习与 $\beta$ 相关的样本形态，同时按 $n$ 分层检验其收益是否稳定，并清楚区分可部署输入、oracle 参照和 hindsight 诊断。
