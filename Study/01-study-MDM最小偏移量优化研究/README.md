...
>
> **当前状态（2026-07-27）**：既有正式实验与技术证据包保持封存。中文源稿 v0.2 复核后，Study01 进入有限扩展验证规划：内部可信性、分轴泛化、Direct-MLP/传统方法公平比较、工程寿命分位点和域匹配真实案例。权威计划见 `07-剩余实验目标与规划.md`，协议见 `02-实验协议.md`；新增证据批准前不冻结论文结论。
