...
>
> **当前状态（2026-07-27）**：既有正式实验与技术证据包保持封存。扩展验证 P0–P1 已获 Codex APPROVE（`be8d7b81`）：完整性通过，现有证据确认缺少纯样本量插值，纯参数插值仅 1 个组合。P2 已冻结为 15 个 `n=15` 纯样本量插值组合和 24 个纯参数插值组合，尚未运行。权威计划见 `07-剩余实验目标与规划.md`，协议见 `02-实验协议.md`；P2 独立批准前不推进 Direct-MLP，也不冻结论文结论。
