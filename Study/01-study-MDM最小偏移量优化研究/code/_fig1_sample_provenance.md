...
> 生成时间：2026-07-07（rev 4：三 panel 升级，新增 Panel B/C 波动诊断）
...
- **Panel B**：δ=0 与 δ=0.1 的 γ̂ 分布（MC R=1000）。横轴 γ̂（乘 1000 对齐 Panel A 语境），纵轴 density。
- **Panel C**：同上的归一化误差 (γ̂-γ)/η 分布。横轴归一化误差，纵轴 density。
...
显示时 Panel B 把 `gamma_hat` 乘 1000，使横轴对齐 Panel A 的 γ=1000 语境；真值标记为 γ=1000。Panel C 的归一化误差 (γ̂-γ)/η 在 eta=1 下即 gamma_hat - 1，直接使用无需变换；真值标记为 0。
...
这些数字与 Panel A 的 trace 机制一致：δ=0.1 在统计上把 γ̂ 从严重低估（811.6）拉回到接近真值（1103.1），|err| 中位数降一半（0.463 → 0.226）。
